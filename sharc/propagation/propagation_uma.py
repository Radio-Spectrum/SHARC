...
class PropagationUMa(Propagation):
    ...
    def get_loss_los(
        ...
        if len(idl[0]):
            loss[idl] = (
                20 * np.log10(distance_3D[idl]) + 20 * np.log10(frequency[idl]) - 27.55
            )
    ...
    def get_los_probability(self, distance_2D: np.array, h_ue: np.array) -> np.array:
        ...
        p_los[idl] = (
            18 / distance_2D[idl]
            + np.exp(-distance_2D[idl] / 63) * (1 - 18 / distance_2D[idl])
        ) * (
            1
            + 1.25
            * c_prime[idl]
            * np.power(distance_2D[idl] / 100, 3)
            * np.exp(-distance_2D[idl] / 150)
        )
