...
class PropagationP528(Propagation):
    ...
    # ------------------------------------------------------------------
    # PropagationPath interface (feat/dataset). Satisfies the abstract
    # Propagation.get_path_loss and adapts to the masked-path API.
    # ------------------------------------------------------------------
    def get_path_loss(
        self,
        params: "Parameters",
        ...
        """Compute P.528 path loss over the active station pairs of ``path``."""
        station_a = path.sta_a
        station_b = path.sta_b
        na = station_a.num_stations
        nb = station_b.num_stations

        distance = station_a.geom.get_3d_distance_to(station_b.geom)  # (na, nb)
        mskd_dist = path.mtx_to_masked(distance)

        # Terminal heights [m], broadcast to the (na, nb) grid then masked.
        hA = np.asarray(station_a.geom.z_global, dtype=float).reshape(na, 1) \
            * np.ones((na, nb))
        hB = np.asarray(station_b.geom.z_global, dtype=float).reshape(1, nb) \
            * np.ones((na, nb))
        mskd_hA_km = path.mtx_to_masked(hA) / 1e3
        mskd_hB_km = path.mtx_to_masked(hB) / 1e3

        f_arr = float(frequency) * np.ones_like(mskd_dist)
        indoor = np.zeros_like(mskd_dist, dtype=bool)

        p528 = params.single_space_station.param_p528
        Tpol = int(p528.Tpol)
        if p528.time_percentage == "RANDOM":
            p_time = 1 + 98 * self.random_number_gen.rand(*mskd_dist.shape)
        else:
            p_time = float(p528.time_percentage) * np.ones_like(mskd_dist)

        mskd_loss = self.get_loss(
            mskd_dist, f_arr, mskd_hA_km, mskd_hB_km, indoor, Tpol, p_time,
        )
