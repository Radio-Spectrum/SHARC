...
class PropagationUMaTest(unittest.TestCase):
    ...
    def test_los_probability(self):
        ...
        los_probability = np.array([[1, 1, 0.74], [1, 0.57, 0.45]])
        npt.assert_allclose(
            self.uma.get_los_probability(distance_2D, h_ue), los_probability, atol=1e-2
        )
    ...
    def test_loss_los(self):
        ...
        shadowing_std = 0
        loss = np.array(
            [[102.32, 108.09, 111.56, 114.05], [115.99, 117.56, 118.90, 120.06]]
        )
        npt.assert_allclose(
        ...
        shadowing_std = 0
        loss = np.array([[62.32, 68.09, 71.56, 74.05], [87.06, 84.39, 83.57, 83.40]])
        npt.assert_allclose(
            self.uma.get_loss_los(
                distance_2D, distance_3D, frequency, h_bs, h_ue, h_e, shadowing_std
            ),
    ...
    def test_loss_nlos(self):
        ...
        shadowing_std = 0
        loss = np.array(
            [[121.58, 132.25, 138.45, 142.70], [148.29, 150.77, 152.78, 154.44]]
        )
        npt.assert_allclose(
        ...
        shadowing_std = 0
        loss = np.array(
            [[120.02, 131.18, 146.13, 141.75], [138.66, 149.83, 151.84, 153.51]]
        )
        npt.assert_allclose(
            self.uma.get_loss_nlos(
                distance_2D, distance_3D, frequency, h_bs, h_ue, h_e, shadowing_std
