...
class AntennaElementImtF1336(object):
    ...
    def __init__(self, par: AntennaPar):
        ...
        if par.element_theta_3db > 0:
            ...
            if self.phi_3db > 120.0:
                sys.stderr.write(
                    "ERROR\nvertical beamwidth must be givem if horizontal beamwidth > 120 degrees"
                )
                sys.exit(1)
            # calculate based on F1336
            self.theta_3db = (31000 * 10 ** (-0.1 * self.g_max)) / self.phi_3db

        # antenna paremeters, according to ITU-R M2292
        self.k_a = 0.7
        self.k_p = 0.7
        self.k_h = 0.7
        self.lambda_k_h = 3 * (1 - 0.5 ** (-self.k_h))
        self.k_v = 0.3
        self.incline_factor = (
            10
            * np.log10(
                ((180 / self.theta_3db) ** 1.5 * (4**-1.5 + self.k_v))
                / (1 + 8 * self.k_p)
            )
            / np.log10(22.5 / self.theta_3db)
        )
        self.x_k = np.sqrt(1 - 0.36 * self.k_v)
        self.lambda_k_v = (
            12 - self.incline_factor * np.log10(4) - 10 * np.log10(4**-1.5 + self.k_v)
        )

        self.g_hr_180 = (
            -12.0
            + 10 * np.log10(1 + 8 * self.k_a)
            - 15 * np.log10(180 / self.theta_3db)
        )
        self.g_hr_0 = 0
