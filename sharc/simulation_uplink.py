...
class SimulationUplink(Simulation):
    ...
    def snapshot(self, *args, **kwargs):
        ...
        self.coupling_loss_imt = self.calculate_coupling_loss(
            self.bs, self.ue, self.propagation_imt
        )
        self.scheduler()
        self.power_control()
    ...
    def power_control(self):
        ...
        if self.parameters.imt.ue_tx_power_control == "OFF":
            ...
            self.ue.tx_power[ue_active] = self.parameters.imt.ue_p_cmax * np.ones(
                len(ue_active)
            )
        else:
    ...
    def calculate_sinr(self):
        ...
        for bs in bs_active:
            ...
            self.bs.thermal_noise[bs] = (
                10
                * np.log10(
                    BOLTZMANN_CONSTANT * self.parameters.imt.noise_temperature * 1e3
                )
            ...
            # calculate I+N
            self.bs.total_interference[bs] = 10 * np.log10(
                np.power(10, 0.1 * self.bs.rx_interference[bs])
    ...
    def calculate_sinr_ext(self):
        ...
        """
        self.coupling_loss_imt_system = self.calculate_coupling_loss(
            self.system, self.bs, self.propagation_system
        )
        ...
        for bs in bs_active:
            ...
            self.bs.ext_interference[bs] = (
                tx_power[bs] - self.coupling_loss_imt_system[active_beams]
            )
    ...
    def calculate_external_interference(self):
        ...
        if self.co_channel:
            self.coupling_loss_imt_system = self.calculate_coupling_loss(
                self.system, self.ue, self.propagation_system
            )

        if self.adjacent_channel:
            self.coupling_loss_imt_system_adjacent = self.calculate_coupling_loss(
                self.system, self.ue, self.propagation_system, c_channel=False
            )
        ...
        for bs in bs_active:
            ...
            if self.co_channel:
                ...
                interference_ue = (
                    self.ue.tx_power[ue] - self.coupling_loss_imt_system[ue]
                )
        ...
        # calculate N
        self.system.thermal_noise = 10 * np.log10(
            BOLTZMANN_CONSTANT * self.system.noise_temperature * 1e3
        ...
        for bs in bs_active:
            ...
            if self.parameters.imt.interfered_with:
                ...
                active_beams = [
                    i
                    for i in range(
                        bs * self.parameters.imt.ue_k,
                        (bs + 1) * self.parameters.imt.ue_k,
                    )
                ]
