...
class TestCoordinateSystem(unittest.TestCase):
    ...
    def test_set_reference_equator_greenwich(self):
        ...
        cs = CoordinateSystem()
        cs.set_reference(0.0, 0.0, 0.0)
        from sharc.satellite.ngso.constants import EARTH_RADIUS_M
        npt.assert_allclose(cs.ref_x, EARTH_RADIUS_M, rtol=1e-4)
        npt.assert_allclose(cs.ref_y, 0.0, atol=1.0)
        npt.assert_allclose(cs.ref_z, 0.0, atol=1.0)

    def test_set_reference_north_pole(self):
        """Setting reference at North Pole produces expected ECEF."""
        cs = CoordinateSystem()
        cs.set_reference(90.0, 0.0, 0.0)
        from sharc.satellite.ngso.constants import EARTH_RADIUS_M
        npt.assert_allclose(cs.ref_x, 0.0, atol=1.0)
        npt.assert_allclose(cs.ref_y, 0.0, atol=1.0)
        npt.assert_allclose(cs.ref_z, EARTH_RADIUS_M, rtol=1e-4)

    def test_roundtrip_transform(self):
        """convert + revert should recover original ECEF coordinates."""
        cs = CoordinateSystem()
        cs.set_reference(-15.0, -47.0, 100.0)
        ...
        x2, y2, z2 = cs.ecef2enu(x_orig, y_orig, z_orig)
        x_rev, y_rev, z_rev = cs.enu2ecef(x2, y2, z2)
    ...
    def test_reference_point_transforms_to_origin(self):
        ...
        cs = CoordinateSystem()
        cs.set_reference(-23.5, -46.6, 800.0)

        x2, y2, z2 = cs.ecef2enu(
            np.array([cs.ref_x]),
            np.array([cs.ref_y]),
            np.array([cs.ref_z]),
        )
    ...
    def test_rotation_only(self):
        ...
        x2, y2, z2 = cs.ecef2enu(v, zeros, zeros, translate=0)
...
class TestShrinkPolygon(unittest.TestCase):
    ...
    def test_shrink_reduces_area(self):
        ...
        shrunk = shrink_lonlat_polygon_by_km(poly, 50)  # 50 km inward
    ...
    def test_shrink_polygon_still_contained(self):
        ...
        shrunk = shrink_lonlat_polygon_by_km(poly, 10)
