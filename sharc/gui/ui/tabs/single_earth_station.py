...
class SingleEarthStationTab:
    ...
    This class handles the UI for configuring a specific Earth Station's parameters,
    including its physical location geometry, antenna radiation patterns, and
    propagation channel models (e.g., ITU-R P.452).
    """

    def __init__(self, app, parent_frame):
        """
        Initializes the SingleEarthStationTab.
    ...
    def _build_ui(self):
        ...
        ttk.Button(actions, text="Save Config", command=self._se_save_config).pack(
            side="left", padx=(0, 6))
        ttk.Button(actions, text="Load Config",
                   command=self._se_load_config).pack(side="left")
        ...
        #
        # We define adjacent channel emissions and spectral masks here to control
        # out-of-band interference.

        add_row_three(frm0, 0, [
            ("frequency [MHz]", ttk.Entry(
                frm0, textvariable=app.se_frequency, width=12)),
            ("bandwidth [MHz]", ttk.Entry(
                frm0, textvariable=app.se_bandwidth, width=12)),
            ("noise_temperature [K]", ttk.Entry(
                frm0, textvariable=app.se_noise_temperature, width=12)),
        ])
        add_row_three(frm0, 1, [
            ("adjacent_ch_reception", ttk.Combobox(frm0, textvariable=app.se_adjacent_ch_reception,
             values=["ACS", "OFF"], width=12, state="readonly")),
            ("adjacent_ch_selectivity [dB]", ttk.Entry(
                frm0, textvariable=app.se_adjacent_ch_selectivity, width=12)),
            ("adjacent_ch_emissions", ttk.Combobox(frm0, textvariable=app.se_adjacent_ch_emissions,
             values=["ACLR", "SPECTRAL_MASK", "OFF"], width=14, state="readonly")),
        ])
        add_row_three(frm0, 2, [
            ("adjacent_ch_leak_ratio [dB]", ttk.Entry(
                frm0, textvariable=app.se_adjacent_ch_leak_ratio, width=12)),
            ("spectral_mask", ttk.Entry(
                frm0, textvariable=app.se_spectral_mask, width=18)),
            ("spurious_emissions [dBm/MHz]", ttk.Entry(frm0,
        ...
        add_row_three(frm0, 3, [
            ...
            ("height [m]", ttk.Entry(
                frm0, textvariable=app.se_height, width=12)),
            ("polarization_loss [dB] (opt.)", ttk.Entry(
                frm0, textvariable=app.se_polarization_loss, width=12)),
        ])

        # --- GEOMETRY ---
        geom = ttk.LabelFrame(root, text="Geometry")
        geom.pack(fill="x", padx=2, pady=4)

        # Location
        loc_box = ttk.LabelFrame(geom, text="Location")
        loc_box.pack(fill="x", padx=2, pady=(6, 6))

        add_row_three(loc_box, 0, [
            ("location.type", ttk.Combobox(loc_box, textvariable=app.se_loc_type,
        ...
        add_row_three(self.loc_fixed, 0, [
            ("x [m]", ttk.Entry(self.loc_fixed,
             textvariable=app.se_loc_fixed_x, width=12)),
            ("y [m]", ttk.Entry(self.loc_fixed, textvariable=app.se_loc_fixed_y,
             width=12)), ("", ttk.Label(self.loc_fixed, text="")),
        ])
        add_row_three(self.loc_cell, 0, [
            ("cell.min_dist_to_bs [m]", ttk.Entry(
                self.loc_cell, textvariable=app.se_loc_cell_min_dist_to_bs, width=14)),
            ("", ttk.Label(self.loc_cell, text="")
             ), ("", ttk.Label(self.loc_cell, text="")),
        ])
        add_row_three(self.loc_net, 0, [
            ("network.min_dist_to_bs [m]", ttk.Entry(
                self.loc_net, textvariable=app.se_loc_network_min_dist_to_bs, width=14)),
            ("", ttk.Label(self.loc_net, text="")
             ), ("", ttk.Label(self.loc_net, text="")),
        ])
        add_row_three(self.loc_ud, 0, [
            ("min_dist_to_center [m]", ttk.Entry(
                self.loc_ud, textvariable=app.se_loc_ud_min_dist_to_center, width=18)),
            ("max_dist_to_center [m]", ttk.Entry(
                self.loc_ud, textvariable=app.se_loc_ud_max_dist_to_center, width=18)), ("", ttk.Label(self.loc_ud, text="")),
        ])
        ...
        # Azimuth / Elevation
        # [Image of azimuth and elevation angles]

        # Defines the pointing direction of the Earth Station antenna relative to the local horizon and true north.

        ae_box = ttk.LabelFrame(
            geom, text="Antenna Pointing (Azimuth / Elevation)")
        ...
        # Azimuth
        ttk.Label(col_az, text="type").grid(
            row=0, column=0, sticky="w", padx=4, pady=(4, 2))
        cb_az = ttk.Combobox(col_az, textvariable=app.se_az_type,
                             values=AZ_EL_TYPES, width=22, state="readonly")
        cb_az.grid(row=0, column=1, sticky="w", padx=4, pady=(4, 2))

        self.az_fixed = ttk.Frame(col_az)
        self.az_ud = ttk.Frame(col_az)
        self.az_fixed.grid(row=1, column=0, columnspan=2,
                           sticky="ew", padx=2, pady=(6, 2))
        self.az_ud.grid(row=1, column=0, columnspan=2,
                        sticky="ew", padx=2, pady=(6, 2))

        ttk.Label(self.az_fixed, text="fixed [deg]").grid(
            row=0, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.az_fixed, textvariable=app.se_az_fixed, width=12).grid(
            row=0, column=1, sticky="w", padx=4, pady=2)

        ttk.Label(self.az_ud, text="min [deg]").grid(
            row=0, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.az_ud, textvariable=app.se_az_ud_min, width=12).grid(
            row=0, column=1, sticky="w", padx=4, pady=2)
        ttk.Label(self.az_ud, text="max [deg]").grid(
            row=1, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.az_ud, textvariable=app.se_az_ud_max, width=12).grid(
            row=1, column=1, sticky="w", padx=4, pady=2)

        self.az_hint = ttk.Label(
            col_az, text="(POINTING_AT_IMT_CENTER: automatic)", foreground="#555")
        self.az_hint.grid(row=2, column=0, columnspan=2,
        ...
        # Elevation
        ttk.Label(col_el, text="type").grid(
            row=0, column=0, sticky="w", padx=4, pady=(4, 2))
        cb_el = ttk.Combobox(col_el, textvariable=app.se_el_type,
                             values=AZ_EL_TYPES, width=22, state="readonly")
        cb_el.grid(row=0, column=1, sticky="w", padx=4, pady=(4, 2))

        self.el_fixed = ttk.Frame(col_el)
        self.el_ud = ttk.Frame(col_el)
        self.el_fixed.grid(row=1, column=0, columnspan=2,
                           sticky="ew", padx=2, pady=(6, 2))
        self.el_ud.grid(row=1, column=0, columnspan=2,
                        sticky="ew", padx=2, pady=(6, 2))

        ttk.Label(self.el_fixed, text="fixed [deg]").grid(
            row=0, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.el_fixed, textvariable=app.se_el_fixed, width=12).grid(
            row=0, column=1, sticky="w", padx=4, pady=2)

        ttk.Label(self.el_ud, text="min [deg]").grid(
            row=0, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.el_ud, textvariable=app.se_el_ud_min, width=12).grid(
            row=0, column=1, sticky="w", padx=4, pady=2)
        ttk.Label(self.el_ud, text="max [deg]").grid(
            row=1, column=0, sticky="w", padx=4, pady=2)
        ttk.Entry(self.el_ud, textvariable=app.se_el_ud_max, width=12).grid(
            row=1, column=1, sticky="w", padx=4, pady=2)

        self.el_hint = ttk.Label(
            col_el, text="(POINTING_AT_IMT_CENTER: n/a for elevation)", foreground="#555")
        self.el_hint.grid(row=2, column=0, columnspan=2,
                          sticky="w", padx=4, pady=(2, 6))
        ...
        # --- ANTENNA ---
        #
        # Defines the gain characteristics of the antenna. The pattern determines
        # how much signal is transmitted/received in the main lobe vs. side lobes.

        ant = ttk.LabelFrame(root, text="Antenna (Pattern + Parameters)")
        ant.pack(fill="x", padx=2, pady=4)

        add_row_three(ant, 0, [
            ("antenna.pattern", ttk.Combobox(ant, textvariable=app.se_ant_pattern,
             values=SUPPORTED_ANTENNA_PATTERNS, width=28, state="readonly")),
            ("antenna.gain [dBi]", ttk.Entry(
                ant, textvariable=app.se_ant_gain, width=12)), ("", ttk.Label(ant, text="")),
        ])

        self.ant_diam = ttk.Frame(ant)
        self.ant_env = ttk.Frame(
            ant)
        self.ant_s672 = ttk.Frame(ant)
        self.ant_f1245 = ttk.Frame(ant)
        self.ant_hint = ttk.Frame(ant)

        for f in (self.ant_diam, self.ant_env, self.ant_s672, self.ant_f1245, self.ant_hint):
            f.grid(row=1, column=0, columnspan=6, sticky="ew", pady=(6, 0))
            f.grid_remove()

        ttk.Label(self.ant_hint, text="Select a pattern to view specific parameters.").grid(
            row=0, column=0, sticky="w")

        # Antenna sub-frames
        add_row_three(self.ant_diam, 0, [("diameter [m]", ttk.Entry(self.ant_diam, textvariable=app.se_ant_diameter, width=12)), (
            "", ttk.Label(self.ant_diam, text="(freq & gain from system)")), ("", ttk.Label(self.ant_diam, text=""))])
        add_row_three(self.ant_env, 0, [("envelope_gain [dB]", ttk.Entry(self.ant_env, textvariable=app.se_ant_envelope_gain, width=12)), (
            "", ttk.Label(self.ant_env, text="")), ("", ttk.Label(self.ant_env, text=""))])
        add_row_three(self.ant_s672, 0, [("antenna_3_dB [deg]", ttk.Entry(self.ant_s672, textvariable=app.se_ant_3db, width=12)), (
            "antenna_l_s [dB] (opt.)", ttk.Entry(self.ant_s672, textvariable=app.se_ant_l_s, width=12)), ("", ttk.Label(self.ant_s672, text=""))])
        add_row_three(self.ant_f1245, 0, [("gain (F1245) [dB]", ttk.Entry(self.ant_f1245, textvariable=app.se_ant_f1245_gain, width=12)), ("diameter [m]", ttk.Entry(
            self.ant_f1245, textvariable=app.se_ant_f1245_diameter, width=12)), ("frequency [MHz]", ttk.Entry(self.ant_f1245, textvariable=app.se_ant_f1245_frequency, width=12))])
        ...
        add_row_three(ch, 0, [
            ("channel_model", ttk.Combobox(ch, textvariable=app.se_channel_model, values=CHANNEL_MODELS,
             width=10, state="readonly")), ("", ttk.Label(ch, text="")), ("", ttk.Label(ch, text="")),
        ])

        #
        # P.452 accounts for diffraction, tropospheric scatter, and ducting, making it
        # suitable for interference analysis over irregular terrain.
        ...
        ttk.Combobox(self.clutter_row, textvariable=app.p452_clutter_type, values=[
                     "one_end", "both_ends"], width=12, state="readonly").grid(row=0, column=2, sticky="w", padx=4, pady=2)
    ...
    def _bind_mouse_scroll(self, widget):
        """Binds mouse scroll events to the widget (Supports Windows, Linux, MacOS)."""
    ...
    def _refresh_az_ui(self, *args):
        ...
        self.az_fixed.grid_remove()
        self.az_ud.grid_remove()
    ...
    def _refresh_el_ui(self, *args):
        ...
        self.el_fixed.grid_remove()
        self.el_ud.grid_remove()
    ...
    def _refresh_ant_ui(self, *args):
        ...
        for f in (self.ant_diam, self.ant_env, self.ant_s672, self.ant_f1245, self.ant_hint):
            f.grid_remove()
        diameter_patterns = {"ITU-R F.699", "ITU-R S.465", "ITU-R S.580",
        ...
        try:
            ...
            if fpath:
                with open(fpath, "w", encoding="utf-8") as f:
                    json.dump(self._se_collect_config(), f, indent=2)
        ...
        try:
            ...
            if fpath:
                with open(fpath, "r", encoding="utf-8") as f:
                    self._se_apply_config(json.load(f))
                messagebox.showinfo(
