...
class PropagationBuildingEntryLoss(Propagation):
    ...
    def get_loss(
        ...
        angle_correction = 0.212 * np.abs(elevation)
...
if __name__ == "__main__":
    ...
    freq_GHz_log = np.arange(-1, 2.1, 0.1)
    ...
    median_loss_traditional = entry_loss.get_loss(
        freq_MHz, 0, prob=0.5, building_class="TRADITIONAL", test=True
    )
    median_loss_therm_eff = entry_loss.get_loss(
        freq_MHz, 0, prob=0.5, building_class="THERMALLY_EFFICIENT", test=True
    )
    plt.semilogx(freq_GHz, median_loss_traditional, "-", label="TRADITIONAL, 0deg")
    plt.semilogx(
        freq_GHz, median_loss_therm_eff, "--", label="THERMALLY_EFFICIENT, 0deg"
    )
    ...
    plt.figure()
    median_loss_traditional = entry_loss.get_loss(
        freq_MHz, 0, prob=0.5, building_class="TRADITIONAL"
    )
    median_loss_therm_eff = entry_loss.get_loss(
        freq_MHz, 0, prob=0.5, building_class="THERMALLY_EFFICIENT"
    )
    ...
    # 45 degrees
    median_loss_traditional = entry_loss.get_loss(
        freq_MHz, 45, prob=0.5, building_class="TRADITIONAL"
    )
    median_loss_therm_eff = entry_loss.get_loss(
        freq_MHz, 45, prob=0.5, building_class="THERMALLY_EFFICIENT"
    )

    plt.semilogx(freq_GHz, median_loss_traditional, "-", label="TRADITIONAL, 45deg")
    plt.semilogx(
        freq_GHz, median_loss_therm_eff, "--", label="THERMALLY_EFFICIENT, 45deg"
    )

    # 90 deg
    median_loss_traditional = entry_loss.get_loss(
        freq_MHz, 90, prob=0.5, building_class="TRADITIONAL"
    )
    median_loss_therm_eff = entry_loss.get_loss(
        freq_MHz, 90, prob=0.5, building_class="THERMALLY_EFFICIENT"
    )

    plt.semilogx(freq_GHz, median_loss_traditional, "-", label="TRADITIONAL, 90deg")
    plt.semilogx(
        freq_GHz, median_loss_therm_eff, "--", label="THERMALLY_EFFICIENT, 90deg"
    )
    ...
    for n, el in enumerate(elevations):
        for m, pb in enumerate(probability):
            loss[n, m] = entry_loss.get_loss(
                freq_MHz, el, prob=pb, building_class="TRADITIONAL"
            )
