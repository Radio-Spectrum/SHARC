...
class PropagationUMi(Propagation):
    ...
    def __init__(
        self, random_number_gen: np.random.RandomState, los_adjustment_factor: float
    ):
    ...
    def get_loss(self, *args, **kwargs) -> np.array:
        ...
        if len(i_nlos[0]):
            loss_nlos = self.get_loss_nlos(
                d_2D, d_3D, f, h_bs, h_ue, h_e, shadowing_nlos
            )
    ...
    def get_loss_los(
        ...
        if len(idl[0]):
            loss[idl] = (
                21 * np.log10(distance_3D[idl]) + 20 * np.log10(frequency[idl]) - 27.55
            )

        if len(idg[0]):
            fitting_term = -9.5 * np.log10(
                breakpoint_distance**2 + (h_bs[:, np.newaxis] - h_ue) ** 2
            )
            loss[idg] = (
                40 * np.log10(distance_3D[idg])
                + 20 * np.log10(frequency[idg])
                - 27.55
                + fitting_term[idg]
            )
    ...
    def get_loss_nlos(
        ...
        loss_nlos = (
            -37.55
            + 35.3 * np.log10(distance_3D)
            + 21.3 * np.log10(frequency)
            - 0.3 * (h_ue - 1.5)
        )
    ...
    def get_breakpoint_distance(
        self, frequency: float, h_bs: np.array, h_ue: np.array, h_e: np.array
    ) -> float:
        """
    ...
    def get_los_probability(
        self, distance_2D: np.array, los_adjustment_factor: float
    ) -> np.array:
        """
        ...
        p_los[idl] = los_adjustment_factor / distance_2D[idl] + np.exp(
            -distance_2D[idl] / 36
        ) * (1 - los_adjustment_factor / distance_2D[idl])
...
if __name__ == "__main__":
    ...
    loss_fs = PropagationFreeSpace(np.random.RandomState(101)).get_loss(
        distance_2D=distance_2D, frequency=freq
    )
