...
class SpectralMask(ABC):
    ...
    def power_calc(self, center_f: float, band: float):
        ...
        inc_df = np.where(
            np.logical_and(self.freq_lim > df_min, self.freq_lim < df_max)
        )[0]
        ...
        # If one or more break limitas are within band
            ...
            if pwr_lvl_1 != self.p_tx:
                power_oob += (self.freq_lim[inc_df[0]] - df_min) * np.power(
                    10, (pwr_lvl_1 / 10)
                )
            if pwr_lvl_2 != self.p_tx:
                power_oob += (df_max - self.freq_lim[inc_df[-1]]) * np.power(
                    10, (pwr_lvl_2 / 10)
                )
            ...
            for df in inc_df[0:-1]:
                ...
                if pwr_lvl != self.p_tx:
                    power_oob += (self.freq_lim[df + 1] - self.freq_lim[df]) * np.power(
                        10, (pwr_lvl / 10)
                    )
