...
class StationFactory(object):
    ...
    def generate_imt_base_stations(
        ...
        imt_base_stations.active = (
            random_number_gen.rand(num_bs) < param.bs_load_probability
        )
        ...
        for i in range(num_bs):
            imt_base_stations.antenna[i] = AntennaBeamformingImt(
                par, imt_base_stations.azimuth[i], imt_base_stations.elevation[i]
            )
    ...
    def generate_imt_ue(
        ...
        if param.topology == "INDOOR":
            return StationFactory.generate_imt_ue_indoor(
                param, param_ant, random_number_gen, topology
            )
        else:
    ...
    def generate_imt_ue_outdoor(
        ...
        if param.ue_distribution_type.upper() == "UNIFORM":
            ...
            if param.ue_distribution_distance.upper() == "RAYLEIGH":
                ...
                sys.stderr.write(
                    ...
                    + param.ue_distribution_distance
                )
                sys.exit(1)
            ...
            if param.ue_distribution_azimuth.upper() == "NORMAL":
                ...
                angle_cutoff = 60
                idx = np.where((angle_n < angle_cutoff) & (angle_n > -angle_cutoff))[0][
                    :num_ue
                ]
                angle = angle_n[idx]
        ...
        imt_ue.indoor = random_number_gen.random_sample(num_ue) <= (
            param.ue_indoor_percent / 100
        )
        ...
        for i in range(num_ue):
            imt_ue.antenna[i] = AntennaBeamformingImt(
                par, imt_ue.azimuth[i], imt_ue.elevation[i]
            )
    ...
    def generate_imt_ue_indoor(
        ...
        for bs in range(num_bs):
            ...
            # Left most cell of first floor
                ...
                bs % topology.num_cells == topology.num_cells - 1
                and bs < topology.total_bs_level
            ):
            ...
            z = [
                topology.height[bs] - topology.b_h + param.ue_height
                for k in range(num_ue_per_bs)
            ]
            ue_x.extend(x)
            ue_y.extend(y)
            ue_z.extend(z)
            ...
            if bs % topology.num_cells == 0:
                out = (
                    (x < topology.x[bs] - topology.cell_radius)
    ...
    def generate_system(
        ...
        if parameters.general.system == "FSS_ES":
            ...
            return StationFactory.generate_haps(
                parameters.haps, parameters.imt.intersite_distance, random_number_gen
            )
    ...
    def generate_fss_space_station(param: ParametersFssSs):
        ...
        sat_lat_rad = param.lat_deg * np.pi / 180.0
        imt_long_diff_rad = param.imt_long_diff_deg * np.pi / 180.0
        ...
        imt_lat_rad = param.imt_lat_deg * np.pi / 180.0
        fss_space_station.x = (
            np.array([x1 * np.sin(imt_lat_rad) - z1 * np.cos(imt_lat_rad)]) * 1000
        )
        fss_space_station.y = np.array([y1]) * 1000
        fss_space_station.height = np.array(
            [
                (
                    z1 * np.sin(imt_lat_rad)
                    + x1 * np.cos(imt_lat_rad)
    ...
    def generate_fss_earth_station(
        param: ParametersFssEs, random_number_gen: np.random.RandomState, *args
    ):
        """
        ...
        if param.location.upper() == "FIXED":
            ...
            x, y, dummy1, dummy2 = StationFactory.get_random_position(
                1, topology, random_number_gen, param.min_dist_to_bs, True
            )
            fss_earth_station.x = np.array(x)
            fss_earth_station.y = np.array(y)
        elif param.location.upper() == "NETWORK":
            x, y, dummy1, dummy2 = StationFactory.get_random_position(
                1, topology, random_number_gen, param.min_dist_to_bs, False
            )
            fss_earth_station.x = np.array(x)
            fss_earth_station.y = np.array(y)
        ...
        if param.azimuth.upper() == "RANDOM":
            fss_earth_station.azimuth = random_number_gen.uniform(-180.0, 180.0)
        else:
    ...
    def generate_rns(param: ParametersRns, random_number_gen: np.random.RandomState()):
        ...
        rns.azimuth = (
            90
            + (azimuth[1] - azimuth[0]) * random_number_gen.random_sample(num_rns)
            + azimuth[0]
        )
        rns.elevation = (elevation[1] - elevation[0]) * random_number_gen.random_sample(
            num_rns
        ) + elevation[0]
    ...
    def generate_ras_station(param: ParametersRas):
        ...
        ras_station.noise_temperature = np.array(
            param.antenna_noise_temperature + param.receiver_noise_temperature
        )
    ...
    def generate_eess_passive_sensor(param: ParametersEessPassive):
        ...
        eess_passive_sensor.y = np.array(
            [distance * math.cos(math.radians(theta_grd_elev))]
        )
        eess_passive_sensor.height = np.array(
...
if __name__ == "__main__":
    ...
    fig = plt.figure(
        figsize=(8, 8), facecolor="w", edgecolor="k"
