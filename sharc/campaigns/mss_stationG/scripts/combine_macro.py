...
dl_dir = os.path.join(campaign_base_dir, "output_data")
...
output_ul_dir = os.path.join(output_combined_dir, "output_ul")
os.makedirs(output_dl_dir, exist_ok=True)
os.makedirs(output_ul_dir, exist_ok=True)
...
def copiar_yaml_para_saida():
    ...
    # Para cada tipo de link (DL e UL)
    for tipo_link, base_dir in [("dl", dl_dir), ("ul", ul_dir)]:
        # Diretório de saída correspondente
        output_dir = output_dl_dir if tipo_link == "dl" else output_ul_dir
...
combinar_e_salvar("ul", ul_dir, output_ul_dir, num_snap=None)
