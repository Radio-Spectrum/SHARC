...
def build_results_tab(self, root):
    ...
    ttk.Checkbutton(frm_auto, text="Automatic update", variable=self.var_auto_update,
    ...
    ttk.Spinbox(frm_auto, from_=500, to=10000, increment=500, textvariable=self.var_update_period_ms, width=8,
                command=self._schedule_auto_update).pack(side="left", padx=(4, 8))
    ttk.Button(frm_auto, text="Update now", command=self._draw_results_plots).pack(side="left")
    ...
    ttk.Button(frm_export, text="Export figure…", command=self._export_results_fig).pack(side="left")
    ...
    # ttk.Button(fexp, text="Export figure…", command=self._export_results_figure).pack(side="left")
    ...
    ttk.Button(ref_row, text="Add", command=self._ref_add).pack(side="left")
    ...
    btns.pack(fill="x")
    ttk.Button(btns, text="Remove selected", command=self._ref_remove).pack(side="left")
    ttk.Button(btns, text="Apply (redraw)", command=self._draw_results_plots).pack(side="left", padx=(6, 0))
