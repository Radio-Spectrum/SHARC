...
# Function to preserve comments while dumping YAML
def write_yaml(filepath, min_elev, max_elev):
    ...
    output_dir: campaigns/mss_d2d_to_imt_elev_study/output/
    ###########################################################################
    # output folder prefix
    output_dir_prefix: output_mss_d2d_to_imt_co_channel_system_A_min_{min_elev}_max_{max_elev}
imt:
    ###########################################################################
    ...
    topology:
        ...
        single_bs:
            ...
            num_clusters: 1
            ###########################################################################
            # Whether the BS is positioned over a sphere (Earth) surface
            is_spherical: true
...
mss_d2d:
    ...
    bandwidth: 5.0
    # Region of visible satellites
    min_elevation_angle_deg:  {min_elev}
    max_elevation_angle_deg:  {max_elev}
    # MSS_D2D cell radius in network topolog"Generated file: {filepath}"y [m]
    cell_radius: 36516.0
    # Satellite power density in dBW/Hz
    tx_power_density: -54.2
    # Number of write_yaml_with_commentssectors
    num_sectors: 19
...
# Generate intervals: (5-10), (10-15), ..., (85-90)
for min_elev in range(5, 90, 5):
    max_elev = min_elev + 5
    filename = f"mss_d2d_to_imt_co_channel_system_A_min_{min_elev}_max_{max_elev}.yaml"
    filepath = os.path.join(current_dir, filename)
    write_yaml(filepath, min_elev, max_elev)
