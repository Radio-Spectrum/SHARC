...
class Simulation(ABC, Observable):
    def __init__(self, parameters: Parameters, parameter_file: str):
        ...
        self.wrap_around_enabled = (
            self.parameters.imt.wrap_around
            and (
                self.parameters.imt.topology == "MACROCELL"
                or self.parameters.imt.topology == "HOTSPOT"
            )
            and self.parameters.imt.num_clusters == 1
        )
        ...
        if (
            self.overlapping_bandwidth == self.param_system.bandwidth
            and not self.parameters.imt.interfered_with
        ) or (
            self.overlapping_bandwidth == self.parameters.imt.bandwidth
            and self.parameters.imt.interfered_with
        ):
        ...
        self.propagation_imt = PropagationFactory.create_propagation(
            self.parameters.imt.channel_model, self.parameters, random_number_gen
        )
        self.propagation_system = PropagationFactory.create_propagation(
            self.param_system.channel_model, self.parameters, random_number_gen
        )
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.bs_power_gain = 10 * math.log10(
            self.parameters.antenna_imt.bs_n_rows
            * self.parameters.antenna_imt.bs_n_columns
        )
        self.ue_power_gain = 10 * math.log10(
            self.parameters.antenna_imt.ue_n_rows
        ...
        self.results = Results(
            self.parameters_filename, self.parameters.general.overwrite_output
        )
    ...
    def calculate_coupling_loss(
        ...
        """
        if (
            station_a.station_type is StationType.EESS_PASSIVE
            or station_a.station_type is StationType.FSS_SS
            or station_a.station_type is StationType.HAPS
            or station_a.station_type is StationType.RNS
        ):
            elevation_angles = station_b.get_elevation_angle(
                station_a, self.param_system
            )
        elif (
            station_a.station_type is StationType.IMT_BS
            and station_b.station_type is StationType.IMT_UE
            and self.parameters.imt.topology == "INDOOR"
        ):
            elevation_angles = np.transpose(station_b.get_elevation(station_a))
        elif (
            station_a.station_type is StationType.FSS_ES
            or station_a.station_type is StationType.RAS
        ):
        ...
        if (
            station_a.station_type is StationType.EESS_PASSIVE
            or station_a.station_type is StationType.FSS_SS
            or station_a.station_type is StationType.FSS_ES
            or station_a.station_type is StationType.HAPS
            or station_a.station_type is StationType.FS
            or station_a.station_type is StationType.RNS
            or station_a.station_type is StationType.RAS
        ):
            ...
            if station_b.station_type is StationType.IMT_UE:
                ...
                gain_b = np.transpose(
                    self.calculate_gains(station_b, station_a, c_channel)
                )
                sectors_in_node = 1
                additional_loss = (
                ...
                # define antenna gains
                gain_a = np.repeat(
                    self.calculate_gains(station_a, station_b),
                ...
                additional_loss = (
                    self.parameters.imt.bs_ohmic_loss + self.polarization_loss
                )
            ...
            if (
                ...
                path_loss = propagation.get_loss(
                    distance_3D=d_3D,
    ...
    def connect_ue_to_bs(self):
        ...
        for bs in bs_active:
            ue_list = [
                i for i in range(bs * num_ue_per_bs, bs * num_ue_per_bs + num_ue_per_bs)
            ]
            ...
            if self.bs.active[bs]:
                ...
                for ue in self.link[bs]:
                    ...
                    self.bs.antenna[bs].add_beam(
                        self.bs_to_ue_phi[bs, ue], self.bs_to_ue_theta[bs, ue]
                    )
    ...
    def scheduler(self):
        ...
        for bs in bs_active:
            ue = self.link[bs]
            self.bs.bandwidth[bs] = (
                self.num_rb_per_ue * self.parameters.imt.rb_bandwidth
            )
    ...
    def calculate_gains(
        self, station_1: StationManager, station_2: StationManager, c_channel=True
    ) -> np.array:
        """
        ...
        if station_1.station_type is StationType.IMT_BS:
            if station_2.station_type is StationType.IMT_UE:
                ...
                station_2.station_type is StationType.EESS_PASSIVE
                or station_2.station_type is StationType.FSS_SS
                or station_2.station_type is StationType.FSS_ES
                or station_2.station_type is StationType.HAPS
                or station_2.station_type is StationType.FS
                or station_2.station_type is StationType.RNS
                or station_2.station_type is StationType.RAS
            ):
            ...
            station_1.station_type is StationType.EESS_PASSIVE
            or station_1.station_type is StationType.FSS_SS
            or station_1.station_type is StationType.FSS_ES
            or station_1.station_type is StationType.HAPS
            or station_1.station_type is StationType.FS
            or station_1.station_type is StationType.RNS
        ...
        if (
            (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.EESS_PASSIVE
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.FSS_SS
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.FSS_ES
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.HAPS
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.FS
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.RNS
            )
            or (
                station_1.station_type is StationType.IMT_BS
                and station_2.station_type is StationType.RAS
            )
        ):
            for k in station_1_active:
                for b in range(
                    k * self.parameters.imt.ue_k, (k + 1) * self.parameters.imt.ue_k
                ):
            ...
            (
                station_1.station_type is StationType.IMT_UE
                and station_2.station_type is StationType.EESS_PASSIVE
            )
            or (
                station_1.station_type is StationType.IMT_UE
            ...
            gains[0, station_2_active] = station_1.antenna[0].calculate_gain(
                phi_vec=phi[0, station_2_active], theta_vec=theta[0, station_2_active]
            )
            ...
            theta = (
                np.degrees(np.arctan((station_1.height - station_2.height) / distance))
                + station_1.elevation
            )
            gains[0, station_2_active] = station_1.antenna[0].calculate_gain(
    ...
    def plot_scenario(self):
        ...
        ax.scatter(
            self.ue.x, self.ue.y, color="r", edgecolor="w", linewidth=0.5, label="UE"
        )
        ...
        for i in range(len(self.ue.x)):
            plt.plot(
                [
                    self.ue.x[i],
                    self.ue.x[i] + d * math.cos(math.radians(self.ue.azimuth[i])),
                ],
                [
                    self.ue.y[i],
                    self.ue.y[i] + d * math.sin(math.radians(self.ue.azimuth[i])),
                ],
                "r-",
            )
