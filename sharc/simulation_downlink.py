    ...
    def calculate_sinr_ext(self):
        ...
        if self.adjacent_channel:
            ...
            if self.param_system.adjacent_ch_emissions != "OFF":
                # TODO: Store this coupling loss for results later.
        ...
        for bs in bs_active:
            ...
            if self.adjacent_channel:
                ...
                if self.parameters.imt.adjacent_ch_emissions != "OFF":
                    ...
                    # same for all beams
                    adj_loss = self.coupling_loss_oob_tx_inband_rx[np.ix_(active_beams, sys_active)]
                ...
                if self.param_system.adjacent_ch_reception != "OFF":
                    rx_oob_s = rx_oob - self.coupling_loss_imt_system_adjacent[active_beams, sys_active]
                else:
                    rx_oob_s = -np.inf
    ...
    def collect_results(self, write_to_file: bool, snapshot_number: int):
        ...
        for bs in bs_active:
            ...
            if self.parameters.imt.interfered_with:  # IMT suffers interference
                ...
                if len(self.system_imt_antenna_gain):
                    self.results.system_imt_antenna_gain.extend(
                        self.system_imt_antenna_gain[sys_active[:, np.newaxis], ue].flatten(),
                    )
                if len(self.system_imt_antenna_gain_adjacent):
