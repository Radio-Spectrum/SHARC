...
class SpectralMaskImt(SpectralMask):
    ...
    def set_mask(self, power=0):
        ...
        if self.sta_type is StationType.IMT_UE:
            ...
            if self.freq_mhz > 24250 and self.freq_mhz < 33400:
                if power >= 34.5:
                    ...
                    # Table 3
                    mask_dbm = np.array(
                        [-5, np.max((power - 47.5, -20)), self.spurious_emissions]
                    )
                ...
                if power >= 32.5:
                    ...
                    # Table 5
                    mask_dbm = np.array(
                        [-5, np.max((power - 45.5, -20)), self.spurious_emissions]
                    )
                ...
                if power >= 30.5:
                    ...
                    # Table 7
                    mask_dbm = np.array(
                        [-5, np.max((power - 43.5, -20)), self.spurious_emissions]
                    )
            else:
        ...
        self.mask_dbm = np.concatenate(
            (mask_dbm[::-1], np.array([self.p_tx]), mask_dbm)
        )
