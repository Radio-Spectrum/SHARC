...
from sharc.propagation.propagation_hdfss_building_side import (
    PropagationHDFSSBuildingSide,
)
...
class PropagationHDFSSBuildingSideTest(unittest.TestCase):
    ...
    def test_get_build_loss(self):
        ...
        build_loss = self.propagation_fixed_value.get_building_loss(sta_type, f, ele)
        self.assertEqual(build_loss, expected_build_loss)

        # Test 2: fixed probability
        expected_build_loss = np.array([[24.4, 33.9, 43.4]])
        build_loss = self.propagation_fixed_prob.get_building_loss(sta_type, f, ele)
        ...
        build_loss = self.propagation_random_prob.get_building_loss(sta_type, f, ele)
        npt.assert_allclose(build_loss, expected_build_loss, atol=1e-1)
    ...
    def test_is_same_build(self):
        ...
        in_build = self.propagation.is_same_building(imt_x, imt_y, es_x, es_y)
    ...
    def test_is_next_build(self):
        ...
        expected_in_build = np.array([True, False, False, False, True])
        in_build = self.propagation.is_next_building(imt_x, imt_y, es_x, es_y)
        npt.assert_array_equal(in_build, expected_in_build)
