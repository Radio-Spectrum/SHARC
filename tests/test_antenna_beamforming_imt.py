...
class AntennaBeamformingImtTest(unittest.TestCase):
    ...
    def test_super_position_vector(self):
        ...
        self.assertTrue(np.allclose(np.real(v_vec), np.real(expected_v_vec), rtol=eps))
        self.assertTrue(np.allclose(np.imag(v_vec), np.imag(expected_v_vec), rtol=eps))
        ...
        v_vec = self.antenna2._super_position_vector(phi, theta)
        expected_v_vec = np.array(
            [[1.0 + 0.0j, -0.912724 + 0.408576j], [1.0 + 0.0j, -0.912724 + 0.408576j]]
        )
        self.assertTrue(np.allclose(np.real(v_vec), np.real(expected_v_vec), rtol=eps))
        self.assertTrue(np.allclose(np.imag(v_vec), np.imag(expected_v_vec), rtol=eps))
    ...
    def test_weight_vector(self):
        ...
        self.assertTrue(np.allclose(np.real(w_vec), np.real(expected_w_vec), rtol=eps))
        self.assertTrue(np.allclose(np.imag(w_vec), np.imag(expected_w_vec), rtol=eps))
        ...
        w_vec = self.antenna2._weight_vector(phi_scan, theta_tilt)
        expected_w_vec = np.array(
            [[0.5 + 0.0j, -0.172870 - 0.469169j], [0.0 + 0.5j, 0.469165 - 0.172870j]]
        )
        self.assertTrue(np.allclose(np.real(w_vec), np.real(expected_w_vec), rtol=eps))
        self.assertTrue(np.allclose(np.imag(w_vec), np.imag(expected_w_vec), rtol=eps))
    ...
    def test_calculate_gain(self):
        ...
        gains = self.antenna2.calculate_gain(
            phi_vec=phi, theta_vec=theta, beams_l=beams_l
        )
        ...
        gains = self.antenna1.calculate_gain(
            phi_vec=phi, theta_vec=theta, co_channel=False
        )
    ...
    def test_normalization(self):
        ...
        gain_ref = self.antenna4.calculate_gain(
            phi_vec=phi_v, theta_vec=theta_v, beams_l=beams_l
        )
