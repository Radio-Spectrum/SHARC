...
def build_victim_tab(self, root):
    ...
    ttk.Button(topbar, text="Save Single Space Station config (.json)", command=self._save_victim_config).pack(side="left")
    ttk.Button(topbar, text="Load Single Space Station config (.json)", command=self._load_victim_config).pack(side="left", padx=(6, 0))
    ...
    add_row_three(frm0, 2, [
        ...
        ("Spherical Earth?", ttk.Checkbutton(
            frm0, variable=self.ss_is_global_cs  # <- just pass the widget, no .grid()
        )),
        ("", ttk.Label(frm0, text="")),
    ])
    ...
    ttk.Label(self.frm_other_ant, text="Parameters for this pattern are not yet implemented.").grid(row=0, column=0, sticky="w")
