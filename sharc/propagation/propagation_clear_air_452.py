...
class PropagationClearAir(Propagation):
    ...
    def closs_corr(f, d, h, zone, htg, hrg, ha_t, ha_r, dk_t, dk_r):
        ...
        if ha > htg:
            ...
            Aht = (
                10.25 * Ffc * np.exp(-dk) * (1 - np.tanh(6 * (htg / ha - 0.625))) - 0.33
            )  # (57)
        ...
        if ha > hrg:
            Ffc = 0.25 + 0.375 * (1 + np.tanh(7.5 * (f - 0.5)))  # (57a)
            Ahr = (
                10.25 * Ffc * np.exp(-dk) * (1 - np.tanh(6 * (hrg / ha - 0.625))) - 0.33
            )  # (57)
        ...
        if (
            index2 - index1 < 3
        ):  # at least two points between the clutter at Tx and Rx sides
    ...
    def dl_bull(d, h, hts, hrs, ap, f):
        ...
        if Stim < Str:  # Case 1, Path is LoS
            ...
            nub = (
                hts + Stim * dbp - (hts * (dtot - dbp) + hrs * dbp) / dtot
    ...
    def get_loss(self, *args, **kwargs) -> np.array:
        ...
        for ii in range(num_dists):
            [dc, hc, zonec, htg, hrg, Aht, Ahr] = self.closs_corr(
                f, d[ii, :], h[ii, :], zone, Hte, Hre, ha_t, ha_r, dk_t, dk_r
            )
            d[ii, :] = dc
            h[ii, :] = hc
            ...
            [
                ...
                theta,
                pathtype,
            ] = self.smooth_earth_heights(d[ii, :], h[ii, :], htg, hrg, ae, f)
            ...
            [Lbfsg, Lb0p, Lb0b] = self.pl_los(
                dtot, f, p[ii], b0[ii], omega[ii], T, Ph, dlt, dlr
            )

            [Ldp, Ld50] = self.dl_p(
                d[ii], h[ii], hts, hrs, hstd, hsrd, f, omega[ii], p[ii], b0[ii], deltaN
            )
