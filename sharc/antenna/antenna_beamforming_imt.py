...
class AntennaBeamformingImt(Antenna):
    ...
    def __init__(self, par: AntennaPar, azimuth: float, elevation: float):
        ...
        if (par.element_pattern).upper() == "M2101":
            ...
            sys.stderr.write(
                "ERROR\nantenna element type {} not supported".format(
                    par.element_pattern
                )
            )
            sys.exit(1)
    ...
    def calculate_gain(self, *args, **kwargs) -> np.array:
        ...
        if co_channel:
            for g in range(n_direct):
                gains[g] = (
                    self._beam_gain(lo_phi_vec[g], lo_theta_vec[g], beams_l[g])
                    + correction_factor[correction_factor_idx[g]]
                )
        else:
            for g in range(n_direct):
                gains[g] = (
                    self.element.element_pattern(lo_phi_vec[g], lo_theta_vec[g])
    ...
    def _super_position_vector(self, phi: float, theta: float) -> np.array:
        ...
        exp_arg = (n[:, np.newaxis] - 1) * self.dv * np.cos(r_theta) + (
            m - 1
        ) * self.dh * np.sin(r_theta) * np.sin(r_phi)
    ...
    def _weight_vector(self, phi_tilt: float, theta_tilt: float) -> np.array:
        ...
        exp_arg = (n[:, np.newaxis] - 1) * self.dv * np.sin(r_theta) - (
            m - 1
        ) * self.dh * np.cos(r_theta) * np.sin(r_phi)

        w_vec = (1 / np.sqrt(self.n_rows * self.n_cols)) * np.exp(
            2 * np.pi * 1.0j * exp_arg
        )
    ...
    def _beam_gain(self, phi: float, theta: float, beam=-1) -> float:
        ...
        if beam == -1:
            ...
            array_g = 10 * np.log10(
                abs(np.sum(np.multiply(v_vec, self.w_vec_list[beam]))) ** 2
            )
    ...
    def to_local_coord(self, phi: float, theta: float) -> tuple:
        ...
        lo_phi = np.ravel(
            np.asarray(np.rad2deg(np.arctan2(rotated_points[1], rotated_points[0])))
        )
...
class PlotAntennaPattern(object):
    ...
    def plot_element_pattern(
        self, antenna: AntennaBeamformingImt, sta_type: str, plot_type: str
    ):
        phi_escan = 0
        theta_tilt = 90
        ...
        if plot_type == "ELEMENT":
            ...
            gain = antenna.calculate_gain(
                phi_vec=phi, theta_vec=theta, beams_l=np.zeros_like(phi, dtype=int)
            )
