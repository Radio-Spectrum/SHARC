...
class Scintillation:
    ...
    def get_tropospheric_attenuation(self, *args, **kwargs) -> np.array:
        ...
        f_GHz = kwargs["frequency_MHz"] / 1000.0
        elevation_rad = kwargs["elevation"] / 180.0 * np.pi
        ...
        if not wet_refractivity:
            ...
            temperature, pressure, water_vapour_density = (
                self.atmosphere.get_reference_atmosphere_p835(
                    sat_params.imt_lat_deg, sat_params.imt_altitude, sat_params.season
                )
            )
            ...
            coef_c = 257.14
            coef_d = 234.5
            EF_water = 1 + 1e-4 * (7.2 + pressure * (0.032 + 5.9e-6 * temperature**2))
            vapour_pressure = (
                water_vapour_density * temperature / 216.7
            )  # eq 10 in P453
            wet_refractivity = (
        ...
        eff_antenna_diameter = 0.3 * 10 ** (0.05 * antenna_gain_dB) / (np.pi * f_GHz)

        x = 1.22 * eff_antenna_diameter**2 * (f_GHz / path_length)
        antenna_averaging_factor = np.sqrt(
            3.86 * (x**2 + 1) ** (11 / 12) * np.sin(11 / 6 * np.arctan(1 / x))
            - 7.08 * x**5 / 6
        )
        scintillation_intensity = (
            sigma_ref
            * f_GHz ** (7 / 12)
            * antenna_averaging_factor
        ...
        time_percentage = time_ratio * 100.0
        ...
        a_ste = (
            2.672
            - 1.258 * np.log10(time_percentage)
            - 0.0835 * np.log10(time_percentage) ** 2
            - 0.0597 * np.log10(time_percentage) ** 3
        )

        a_stf = (
            3.0
            - 1.71 * np.log10(100 - time_percentage)
            + 0.072 * np.log10(100 - time_percentage) ** 2
            - 0.061 * np.log10(100 - time_percentage) ** 3
        )

        gain_indices = np.where(time_percentage <= 50.0)[0]
        if gain_indices.size:
            attenuation[gain_indices] = (
                -scintillation_intensity[gain_indices] * a_ste[gain_indices]
            )
        fade_indices = np.where(time_percentage > 50.0)[0]
        if fade_indices.size:
            attenuation[fade_indices] = (
                scintillation_intensity[fade_indices] * a_stf[fade_indices]
            )
...
if __name__ == "__main__":
    ...
    percentage_fading_exceeded = 10 ** np.arange(-2, 1.1, 0.1)
    antenna_gain = 0.0
    frequency_MHz = 30000.0
    wet_refractivity = 42.5

    elevation_vec = np.array([5.0, 10.0, 20.0, 35.0, 90.0])
    ...
    for elevation in elevation_vec:
        ...
        plt.semilogx(
            percentage_fading_exceeded, attenuation, label="{} deg".format(elevation)
        )

    percentage_gain_exceeded = 10 ** np.arange(-2, 1.1, 0.1)
    for elevation in elevation_vec:
        attenuation = propagation.get_tropospheric_attenuation(
            elevation=elevation,
            frequency_MHz=frequency_MHz,
