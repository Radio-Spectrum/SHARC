...
row_type.grid(row=0, column=0, columnspan=6, sticky="we", pady=(0,4))
  ...
  ("central_latitude", ttk.Entry(frm_t, textvariable=self.topo_c_lat, width=12)),
    ...
    # Linha 0: raster_encoding + dist_type (ACIMA da lista de 
countries)
   ...
   cb_dist = ttk.Combobox(
    ...
    row_c.grid(row=1, column=0, 
columnspan=6, sticky="we", pady=2)
    ...
    add_row_three(self.frm_t_countries, 2, [
        ...
        ("cell_radius [m]", 
ttk.Entry(self.frm_t_countries, textvariable=self.topo_cell_radius, width=10)),
...
t == "MACROCELL":
    ...
    frm_bs = ttk.LabelFrame(root, text="BS 
– Parâmetros")
    ...
    self._add_field(col_basic, 2, "height 
[m]",
    ...
    self._add_field(col_basic, 4, "ohmic_loss [dB]",
                    ttk.Entry(col_basic, textvariable=self.bs_ohmic, 
width=10))
    ...
    # normalization (use 
Checkbutton para evitar "0/1" na UI)
...
width=18)
 ...
 self._add_field(col_basic_ue, 7, "p_cmax [dBm]", ttk.Entry(col_basic_ue, textvariable=self.ue_p_cmax, width=10))
    ...
    col_array_ue 
= ttk.LabelFrame(frm_ue, text="UE – Array da Antena")
    ...
    col_dist_ue.grid(row=1, column=0, sticky="nsew", padx=(3,6), pady=(0,6))  # 
...
where possible
        pass
    S("imt_min_sep", self.imt_min_sep);
S("imt_interfered", self.imt_interfered)
    S("imt_freq", self.imt_freq); S("imt_bw", self.imt_bw);
S("imt_rb_bw", self.imt_rb_bw)
    S("imt_spec_mask", self.imt_spec_mask);
S("imt_spurious", self.imt_spurious)
    S("imt_adj_ant_model", self.imt_adj_ant_model);
S("imt_guard_ratio", self.imt_guard_ratio)
    S("topo_c_lat", self.topo_c_lat); S("topo_c_lon", self.topo_c_lon);
S("topo_c_alt", self.topo_c_alt)
    S("topo_type", self.topo_type);
S("topo_dist_type", self.topo_dist_type)
    S("topo_num_bs", self.topo_num_bs); S("topo_cell_radius", self.topo_cell_radius);
S("topo_rng", self.topo_rng)
    if "countries" in vals:
        self.txt_countries.delete("1.0","end");
self.txt_countries.insert("1.0", vals["countries"])
    S("path_shp", self.path_shp);
S("path_raster", self.path_raster)
    S("raster_encoding", self.raster_encoding);
S("sedac_mode", self.sedac_mode)
    S("sedac_min", self.sedac_min); S("sedac_max", self.sedac_max);
S("pixel_area_method", self.pixel_area_method)
    # BS
    S("bs_load_prob", self.bs_load_prob);
S("bs_power", self.bs_power); S("bs_height", self.bs_height)
    S("bs_nf", self.bs_nf); S("bs_ohmic", self.bs_ohmic);
S("bs_norm", self.bs_norm)
    S("bs_elem_pat", self.bs_elem_pat);
S("bs_min_arr_gain", self.bs_min_arr_gain)
    S("bs_downtilt", self.bs_downtilt);
S("bs_elem_max_g", self.bs_elem_max_g)
    S("bs_phi3", self.bs_phi3);
S("bs_theta3", self.bs_theta3)
    S("bs_rows", self.bs_rows);
S("bs_cols", self.bs_cols)
    S("bs_elem_hs", self.bs_elem_hs);
S("bs_elem_vs", self.bs_elem_vs)
    S("bs_elem_am", self.bs_elem_am);
S("bs_elem_sla_v", self.bs_elem_sla_v)
    S("bs_mult", self.bs_mult);
S("bs_sub_enabled", self.bs_sub_enabled)
    S("bs_sub_rows", self.bs_sub_rows);
S("bs_sub_evspace", self.bs_sub_evspace)
    S("bs_sub_e_downtilt", self.bs_sub_e_downtilt)
    # UE
    S("ue_k", self.ue_k);
S("ue_km", self.ue_km); S("ue_indoor", self.ue_indoor); S("ue_dist_type", self.ue_dist_type)
    S("ue_tx_power_ctrl", self.ue_tx_power_ctrl);
S("ue_p_o_pusch", self.ue_p_o_pusch)
    S("ue_alpha", self.ue_alpha); S("ue_p_cmax", self.ue_p_cmax);
S("ue_p_dyn", self.ue_p_dyn)
    S("ue_height", self.ue_height); S("ue_nf", self.ue_nf); S("ue_ohmic", self.ue_ohmic);
S("ue_body_loss", self.ue_body_loss)
    S("ue_norm", self.ue_norm); S("ue_elem_pat", self.ue_elem_pat);
S("ue_min_arr_gain", self.ue_min_arr_gain)
    S("ue_elem_max_g", self.ue_elem_max_g); S("ue_phi3", self.ue_phi3);
S("ue_theta3", self.ue_theta3)
    S("ue_rows", self.ue_rows); S("ue_cols", self.ue_cols);
S("ue_elem_am", self.ue_elem_am)
    S("ue_elem_sla_v", self.ue_elem_sla_v);
S("ue_mult", self.ue_mult)
    # UL/DL
    S("ul_att", self.ul_att);
S("ul_sinr_min", self.ul_sinr_min); S("ul_sinr_max", self.ul_sinr_max)
    S("dl_att", self.dl_att); S("dl_sinr_min", self.dl_sinr_min);
S("dl_sinr_max", self.dl_sinr_max)
    S("ch_model", self.ch_model);
S("shadowing", self.shadowing)
