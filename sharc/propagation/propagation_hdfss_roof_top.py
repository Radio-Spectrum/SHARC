...
class PropagationHDFSSRoofTop(Propagation):
    ...
    def __init__(
        self, param: ParametersFssEs, random_number_gen: np.random.RandomState
    ):
    ...
    def get_loss(self, *args, **kwargs) -> np.array:
        ...
        same_build = self.is_same_building(imt_x, imt_y, es_x, es_y)
        ...
        fspl_to_los_bool = np.logical_and(
            d > self.fspl_dist, d <= self.fspl_to_los_dist
        )
        ...
        if not self.param.same_building_enabled:
            ...
            loss[:, same_build_idx] += self.get_same_build_loss(
                imt_z[same_build_idx], es_z
            )

        loss[:, fspl_idx] += self.propagation_fspl.get_loss(
            distance_3D=d[:, fspl_idx], frequency=f[:, fspl_idx]
        )
        loss[:, fspl_to_los_idx] += self.interpolate_fspl_to_los(
            d[:, fspl_to_los_idx], f[:, fspl_to_los_idx], self.param.shadow_enabled
        )
        ...
        loss[:, los_to_nlos_idx] += self.interpolate_los_to_nlos(
            d[:, los_to_nlos_idx], f[:, los_to_nlos_idx], self.param.shadow_enabled
        )
        ...
        if self.param.building_loss_enabled:
            ...
            build_loss[0, not_same_build] = self.get_building_loss(
                imt_sta_type, f[:, not_same_build], elevation[:, not_same_build]
            )
        else:
            build_loss = 0.0
    ...
    def interpolate_fspl_to_los(self, dist, freq, shad):
        fspl_loss = self.propagation_fspl.get_loss(
            distance_3D=self.fspl_dist, frequency=freq
        )
        los_loss = self.propagation_p1411.get_loss(
            distance_3D=self.fspl_to_los_dist, frequency=freq, los=True, shadow=False
        )
    ...
    def interpolate_los_to_nlos(self, dist, freq, shad):
        los_loss = self.propagation_p1411.get_loss(
            distance_3D=self.los_dist, frequency=freq, los=True, shadow=False
        )
        nlos_loss = self.propagation_p1411.get_loss(
            distance_3D=self.los_to_nlos_dist, frequency=freq, los=False, shadow=False
        )
    ...
    def is_same_building(self, imt_x, imt_y, es_x, es_y):
        building_x_range = es_x + (1 + self.b_tol) * np.array(
            [-self.b_w / 2, +self.b_w / 2]
        )
        building_y_range = es_y + (1 + self.b_tol) * np.array(
            [-self.b_d / 2, +self.b_d / 2]
        )
    ...
    def get_diffraction_loss(self, h, d1, d2, f):
        ...
        loss[v_idx] = 6.9 + 20 * np.log10(
            np.sqrt(np.power((v[v_idx] - 0.1), 2) + 1) + v[v_idx] - 0.1
        )
        return loss
...
if __name__ == "__main__":
    ...
    loss_interp = prop.get_loss(
        distance_3D=d, frequency=f, elevation=ele, imt_sta_type=sta_type
    )
