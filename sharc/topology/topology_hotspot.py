...
class TopologyHotspot(Topology):
    ...
    def __init__(
        self, param: ParametersHotspot, intersite_distance: float, num_clusters: int
    ):
        """
    ...
    def calculate_coordinates(self, random_number_gen=np.random.RandomState()):
        ...
        for cell_x, cell_y, cell_azimuth in zip(
            ...
            macro_cell_x = cell_x + self.macrocell.intersite_distance / 3 * math.cos(
                math.radians(cell_azimuth)
            )
            macro_cell_y = cell_y + self.macrocell.intersite_distance / 3 * math.sin(
                math.radians(cell_azimuth)
            )
            ...
            r = np.maximum(
                0,
                (self.macrocell.intersite_distance / 3) * np.sqrt(3) / 2
                - self.param.max_dist_hotspot_ue / 1.0,
            )
        ...
        for a in range(len(azimuth_values)):
            points.append(
                (
                    candidate_x
                    + radius
                    * math.cos(np.radians(candidate_azimuth + azimuth_values[a])),
                    candidate_y
                    + radius
                    * math.sin(np.radians(candidate_azimuth + azimuth_values[a])),
                )
            )
    ...
    def plot(self, ax: matplotlib.axes.Axes):
        ...
        # plot hotspots
        plt.scatter(
            self.x, self.y, color="g", edgecolor="w", linewidth=0.5, label="Hotspot"
        )
