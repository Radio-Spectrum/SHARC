...
class SpectralMask3Gpp(SpectralMask):
    ...
    def get_frequency_limits(self, sta_type: StationType, bandwidth: float) -> np.array:
        """
        ...
        if sta_type is StationType.IMT_BS:
            ...
            delta_f_lim = np.arange(0, 5.1, 0.1)
    ...
    def set_mask(self, power=0):
        emission_limits = self.get_emission_limits(
            self.sta_type, self.band_mhz, self.spurious_emissions
        )
        ...
        self.mask_dbm = np.concatenate(
            (emission_limits_flipped, np.array([self.p_tx]), emission_limits)
        )

    def get_emission_limits(
        self, sta_type: StationType, bandwidth: float, spurious_emissions: float
    ) -> np.array:
        if sta_type is StationType.IMT_BS:
            # emission limits in dBm/MHz
            emission_limits = 3 - 7 / 5 * (np.arange(0.05, 5, 0.1) - 0.05)
