...
from sharc.support.sharc_geom_countries import (
    GeometryConverter, lla_to_ecef_wgs84, lla_to_local_enu,
)
...
class TopologyCountries(Topology):
    ...
    def __init__(self,
        ...
        # Initialize the base Topology so that attributes such as
        # determines_local_geometry, indoor and is_space_station exist.
    ...
    def calculate_coordinates(self,
        ...
        if self.database.from_db_topology_countries:
            ...
            # Convert to local ENU (simulation) coordinates. Anchor the ENU
            # origin at the geometry_converter reference (same reference used to
            # place the other system, e.g. the space station) so that the BS and
            # the other system share a consistent coordinate frame. Fall back to
            # the centroid of the base stations if no reference was set.
            gc = self.geometry_converter
            if gc is not None and getattr(gc, "ref_lat", None) is not None:
                ref_lat, ref_lon = gc.ref_lat, gc.ref_long
            else:
                ref_lat = float(np.mean(self.lats))
                ref_lon = float(np.mean(self.lons))
            x, y, z = self._lla_to_local_enu(
                self.lats, self.lons, self.height, ref_lat, ref_lon)
            self.x = np.array(x)
            self.y = np.array(y)
            self.z = np.array(z)
        ...
        return self
    # ---------- helpers ----------
    # NOTE: the actual math lives in sharc.support.sharc_geom_countries so that
    # other systems (e.g. the single earth station) can be placed on exactly the
    # same local plane as these base stations.
    _lla_to_ecef = staticmethod(lla_to_ecef_wgs84)
    _lla_to_local_enu = staticmethod(lla_to_local_enu)
