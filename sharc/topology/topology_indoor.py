        ...
        if not self.static_base_stations:
            ...
            for f in range(self.num_floors):
                ...
                self.height = np.concatenate(
                    (self.height, (f + 1) * self.b_h * np.ones_like(floor_x))
                )
    ...
    def plot_top_view(self, ax: matplotlib.axes.Axes):
        ...
        for b in range(int(self.num_base_stations / self.num_cells)):
            ...
            for q in range(8):
                ...
                points.extend(
                    [[x_b, y_b], [x_b + 15, y_b], [x_b + 15, y_b + 15], [x_b, y_b + 15]]
                )
                sector = plt.Polygon(points, fill=None, edgecolor="k")
                ax.add_patch(sector)
        ...
        ax.scatter(
            self.x, self.y, color="k", edgecolor="k", linewidth=2, label="Base station"
        )
    ...
    def plot_side_view(self, ax: matplotlib.axes.Axes):
        ...
        for f in range(int(self.num_floors)):
            for build in self.imt_buildings:
                c = build[1]
                x_b = (
                    self.x[f * self.total_bs_level + c * self.num_cells]
                    - self.cell_radius
                )
...
if __name__ == "__main__":
    ...
    # Plot top view
    fig = plt.figure(
        figsize=(10, 8), facecolor="w", edgecolor="k"
