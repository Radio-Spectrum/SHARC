...
# from ..core.geodesy import lla_to_ecef, WGS84_A
...
#
# # Utils
# from ..utils.preview_3d import (
#     add_polyline3d, draw_bs_post, set_equal_3d,
#     add_wedge_outline3d, sector_polygon_xy, add_sector3d
# )
          ...
          # ---------- Ganho S.672 ----------
           ...
           gain[~los_mask] = -np.inf
            ...
            try:
                ...
                vmax_txt = (self.var_gain_vmax.get() or 
"auto").strip().lower()
            except Exception:
...
auto_vmax = float(np.nanmax(gfinite))
            ...
            vmin = auto_vmin if (vmin_txt in ("auto","")) else 
float(vmin_txt)
 ...
 geoconv = GeometryConverter()
                ...
                self.ax3d.scatter(x, y, z, c="tab:red", 
s=6, depthshade=False, label="BS (countries)", zorder=10)
        ...
        try:
           ...
           self.ax3d.scatter([sx],[sy],[sz], c="tab:purple", s=60, marker="^", depthshade=False, label="Spacecraft (FIXED)", zorder=7)
            ...
            # Link/boresight (S -> ES)
            self.ax3d.plot([sx, ex], [sy, ey], [sz, 
ez], color="tab:purple", lw=1.6, alpha=0.9, label="Pointing to ES", zorder=6)
        except Exception:
            pass
...
self.ax3d.set_ylim([-R, R]); self.ax3d.set_zlim([-R, R])
        self.ax3d.set_box_aspect([1,1,1])
        self.ax3d.set_xlabel("X [m]");
self.ax3d.set_ylabel("Y [m]"); self.ax3d.set_zlabel("Z [m]")
    ...
    try:
        ...
        # um quadradão default;
limites serão ajustados depois
...
all_y.append(y)
        ...
        # Escala 
igual em x,y,z (mastros ficam "baixos")
  ...
  return
        ...
        cell_radius = cr  # usamos no desenho 
da “pizza”
...
lw=1.0)
  ...
  self._set_equal_3d(self.ax3d, xs, ys, z_top=bs_height, margin=0.12)
        self.ax3d.set_xlabel("x [m]");
self.ax3d.set_ylabel("y [m]"); self.ax3d.set_zlabel("z [m] (altura)")
  ...
  self.canvas3d.draw_idle()
    ...
    (xlim, ylim) = self._auto_xy_lim(xs, 
ys, margin=0.18)
...
def _zoom_preview_3d(self, factor):
    """Zoom no 3D: factor>1 dá zoom out;
<1 dá zoom in."""
    try:
...
def _write_yaml_combos(self, root, outdir, combos):
    ...
    for combo in combos:
        ...
        # aplica 
placeholders em TODO o dicionário (strings apenas)
