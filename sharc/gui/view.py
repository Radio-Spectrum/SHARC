...
class View(tkinter.Tk, Observer):
    ...
    def initialize(self):
        ...
        self.__scrolledtext = ThreadSafeScrolledText(
            self.__frame, wrap=tkinter.WORD, width=80, height=25, bd=5
        )
        ...
        self.__stop_button = tkinter.Button(
            self.__frame,
            text="STOP  ",
            image=self.__stop_image,
            compound=tkinter.LEFT,
            state=tkinter.DISABLED,
            command=self.__on_stop_button_click,
        )
        ...
        self.__copy_button = tkinter.Button(
            self.__frame,
            text="COPY",
            image=self.__copy_image,
            compound=tkinter.LEFT,
            state=tkinter.NORMAL,
            command=self.__on_copy_button_click,
        )
    ...
    def __on_start_button_click(self, *args):
        ...
        param_file = tkinter.filedialog.askopenfilename(
            ...
            filetypes=(("Simulation parameters", "*.ini"), ("All files", "*.*")),
        )
        if param_file:
            self.__controller.action(
                action=Action.START_SIMULATION, param_file=param_file
            )
    ...
    def __popup(self, message: str):
        ...
        label = tkinter.Label(top_level, text=message, height=0, width=50, bg="#FFFFFF")
        label.pack()
