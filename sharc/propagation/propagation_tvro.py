...
class PropagationTvro(Propagation):
    ...
    def __init__(self, random_number_gen: np.random.RandomState, environment: str):
        ...
        if "imt_sta_type" in kwargs.keys():
            ...
            if imt_sta_type is StationType.IMT_BS:
                loss = self.get_loss_macrocell(
                    distance_3D, frequency, height, indoor_stations, shadowing
                )
            else:
                loss = self.get_loss_microcell(
                    distance_3D, frequency, indoor_stations, shadowing
                )
                pass
        else:
    ...
    def get_loss_microcell(
        ...
        if shadowing:
            shadowing_fading = self.random_number_gen.normal(0, 3.89, loss.shape)
            loss = loss + shadowing_fading

        loss = loss + self.building_loss * indoor_stations

        free_space_path_loss = self.free_space_path_loss.get_loss(
    ...
    def get_loss_macrocell(
        ...
        f_fc = 0.25 + 0.375 * (1 + np.tanh(7.5 * (frequency / 1000 - 0.5)))
        clutter_loss = (
            10.25
            * f_fc
            * np.exp(-self.d_k)
            * (1 - np.tanh(6 * (height / self.h_a - 0.625)))
            - 0.33
        )
        ...
        if shadowing:
            shadowing_fading = self.random_number_gen.normal(
                0, self.shadowing_std, loss.shape
            )
            loss = loss + shadowing_fading
    ...
    def get_los_probability(
        self, distance: np.array, distance_transition: float = 70
    ) -> np.array:
        """
...
if __name__ == "__main__":
    ...
    loss_urban_bs_ue = prop_urban.get_loss(
    ...
    loss_suburban_bs_ue = prop_suburban.get_loss(
        distance_3D=distance_3D,
        frequency=frequency,
        indoor_stations=indoor_stations,
        shadowing=shadowing,
    ...
    loss_urban_bs_tvro = prop_urban.get_loss(
        distance_3D=distance_3D,
        frequency=frequency,
        indoor_stations=indoor_stations,
        shadowing=shadowing,
    ...
    loss_suburban_bs_tvro = prop_suburban.get_loss(
        distance_3D=distance_3D,
        frequency=frequency,
        indoor_stations=indoor_stations,
        shadowing=shadowing,
    ...
    loss_ue_tvro = prop_urban.get_loss(
        distance_3D=distance_3D,
        frequency=frequency,
        indoor_stations=indoor_stations,
    ...
    loss_fs = prop_free_space.get_loss(distance_3D=distance_3D, frequency=frequency)

    fig = plt.figure(figsize=(7, 5), facecolor="w", edgecolor="k")
    ax = fig.gca()

    ax.semilogx(
        distance_3D, loss_urban_bs_tvro, "-r", label="urban, BS-to-TVRO", linewidth=1
    )
    ax.semilogx(
        distance_3D,
        loss_suburban_bs_tvro,
        "--r",
        label="suburban, BS-to-TVRO",
        linewidth=1,
    )
    ax.semilogx(
        distance_3D, loss_urban_bs_ue, "-b", label="urban, BS-to-UE", linewidth=1
    )
    ax.semilogx(
        distance_3D, loss_suburban_bs_ue, "--b", label="suburban, BS-to-UE", linewidth=1
    )
