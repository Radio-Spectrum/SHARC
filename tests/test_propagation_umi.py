...
class PropagationUMiTest(unittest.TestCase):
    ...
    def test_los_probability(self):
        distance_2D = np.array([[10, 15, 40], [17, 60, 80]])
        los_probability = np.array([[1, 1, 0.631], [1, 0.432, 0.308]])
        npt.assert_allclose(
            self.umi.get_los_probability(distance_2D, self.umi.los_adjustment_factor),
            los_probability,
            atol=1e-2,
        )
    ...
    def test_breakpoint_distance(self):
        ...
        breakpoint_distance = np.array(
            [[11200, 16800], [15200, 22800], [19200, 28800], [23200, 34800]]
        )
    ...
    def test_loss_los(self):
        distance_2D = np.array([[100, 200, 300, 400], [500, 600, 700, 800]])
        h_bs = np.array([30, 35])
        ...
        shadowing_std = 0
        loss = np.array(
            [[104.336, 110.396, 114.046, 116.653], [118.690, 120.346, 121.748, 122.963]]
        )
        npt.assert_allclose(
            self.umi.get_loss_los(
                distance_2D, distance_3D, frequency, h_bs, h_ue, h_e, shadowing_std
            ),
            loss,
            atol=1e-2,
        )
        ...
        shadowing_std = 0
        loss = np.array(
            [[64.336, 70.396, 74.046, 76.653], [89.215, 86.829, 86.187, 86.139]]
        )
        npt.assert_allclose(
            self.umi.get_loss_los(
                distance_2D, distance_3D, frequency, h_bs, h_ue, h_e, shadowing_std
    ...
    def test_loss_nlos(self):
        ...
        shadowing_std = 0
        loss = np.array(
            [[128.84, 138.72, 144.56, 148.64], [152.96, 155.45, 157.50, 159.25]]
        )
        npt.assert_allclose(
        ...
        distance_2D = np.array([[1000, 2000, 5000, 4000], [3000, 6000, 7000, 8000]])
        h_bs = np.array([30, 35])
        h_ue = np.array([2, 3, 4, 5])
        h_e = np.ones(distance_2D.shape)
        distance_3D = np.sqrt(distance_2D**2 + (h_bs[:, np.newaxis] - h_ue) ** 2)
        ...
        shadowing_std = 0
        loss = np.array(
            [[120.96, 131.29, 145.03, 141.31], [137.80, 148.13, 150.19, 151.94]]
        )
        npt.assert_allclose(
            self.umi.get_loss_nlos(
                distance_2D, distance_3D, frequency, h_bs, h_ue, h_e, shadowing_std
