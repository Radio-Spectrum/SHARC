...
general:
  ...
  output_dir: campaigns/imt_mss_hub_type_6_7300/output/   # Output directory (relative to the SHARC/sharc folder)
  output_dir_prefix: output_imt_mss_hub_type_6_7300_y_{y}_elevation_{elevation} # Output directory prefix
imt:
  topology:
  ...
  adjacent_antenna_model: SINGLE_ELEMENT          # Antenna model for adjacent interference: SINGLE_ELEMENT or BEAMFORMING
  spectral_mask: "IMT-2020"                    # Spectral mask: "IMT-2020" or "3GPP E-UTRA"
  ...
  bs:
    ...
    height: 18                                    # BS height [m] 
    noise_figure: 6                               # BS noise figure [dB] (6 dB (Wide Area BS) 11 dB (Medium Range BS) 14 dB (Local Area BS)) from table 4 Annex 4.2 Doc 5D/413-E 15 october 2024
    noise_temperature: 290                        # Noise temperature [K]
    ohmic_loss: 0                                 # BS array ohmic loss [dB] NOTE: Table 10 Note 2 specifies element_gain already includes ohmic loss
    antenna:
      normalization: FALSE                        # Whether normalization should be applied
  ...
  ue:
    ...
    indoor_percent: 70                             # Percentage of indoor UEs [%]
    distribution_type: UNIFORM                    # UE distribution type: UNIFORM or ANGLE_AND_DISTANCE
    ...
    noise_figure: 13                               # UE noise figure [dB] (from table 4 Annex 4.2 Doc 5D/413-E 15 october 2024)
    ohmic_loss: 0                                 # UE ohmic loss [dB]
    body_loss: 4                                  # UE body loss [dB]
    antenna:
      normalization: FALSE                        # Whether normalization should be applied for UE
      normalization_file: antenna/beamforming_normalization/ue_norm.npz
      element_pattern: FIXED                        # UE element radiation pattern
      minimum_array_gain: -200                     # Minimum array gain [dBi]
      element_max_g: -4                            # Maximum element gain [dBi]
      element_phi_3db: 180                         # Horizontal 3dB beamwidth [degrees]
      element_theta_3db: 360                         # Vertical 3dB beamwidth [degrees]
      ...
      multiplication_factor: 12                   # Multiplication factor for pattern adjustment
  downlink:
    attenuation_factor: 0.6                       # Downlink attenuation factor
    sinr_min: -10                                 # Minimum downlink SINR [dB]
    sinr_max: 30                                  # Maximum downlink SINR [dB]
  uplink:
  ...
  channel_model: UMa                             # Channel model: FSPL, CI, UMa, UMi, etc.
  shadowing: TRUE                                # Enable shadowing
 
single_earth_station:
  frequency: 7300                               # Frequency [MHz]
  bandwidth: 1                                   # Bandwidth fixed at 1 MHz
  noise_temperature: 250                        # Noise temperature [K]
  tx_power_density: 0                           # Transmit power spectral density [dBW/Hz]
  adjacent_ch_selectivity: 20.0                  # Adjacent channel selectivity [dB]
  geometry:
    height: 1                                  # Antenna height [m]
    azimuth:
      type: FIXED                             # Azimuth distribution type
      fixed: 180                              # Azimuth angle [degrees]
    elevation:
      type: FIXED
      fixed: {elevation}                          # Placeholder for elevation value
    location:
      type: FIXED
      fixed:
        x: 0
  ...
  channel_model: P452
  param_p425:
    atmospheric_pressure: 1017                  # Total air pressure [hPa]
    air_temperature: 293.15                      # Air temperature [K]
    N0: 352.58                                # Sea-level surface refractivity 
    delta_N: 60                               # Average radio-refractive
...
y_values = [300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600]  # Values for 'y'
elevations = [30]  # Values for 'elevation'
