...
def lla_to_ecef_wgs84(lat_deg, lon_deg, height):
...
def lla_to_local_enu(lat_deg, lon_deg, height, ref_lat, ref_lon):
    """Geodetic (deg, deg, m) -> local ENU (m) relative to (ref_lat, ref_lon).

    Geo -> ECEF -> ENU conversion (WGS-84) so that stations live in the same
    flat local plane the simulator expects, anchored at the given reference
    point. The vertical coordinate is kept as the station height, so Earth
    curvature is not baked into the geometry (the terrestrial propagation
    models account for it themselves).

    Parameters
    ----------
    lat_deg, lon_deg : array-like
        Station geodetic latitude and longitude in degrees.
    height : array-like
        Station height in meters, returned unchanged as the vertical coordinate.
    ref_lat, ref_lon : float
        Latitude and longitude of the local plane origin, in degrees.

    Returns
    -------
    tuple
        East, North and height coordinates in meters.
    """
    lat = np.asarray(lat_deg, dtype=float)
    lon = np.asarray(lon_deg, dtype=float)
    h = np.asarray(height, dtype=float)

    rlat = np.radians(float(ref_lat))
    rlon = np.radians(float(ref_lon))
    Rx, Ry, Rz = lla_to_ecef_wgs84(ref_lat, ref_lon, 0.0)

    Xp, Yp, Zp = lla_to_ecef_wgs84(lat, lon, h)
    dx, dy, dz = Xp - Rx, Yp - Ry, Zp - Rz

    sl, cl = np.sin(rlat), np.cos(rlat)
    sb, cb = np.sin(rlon), np.cos(rlon)
    east = -sb * dx + cb * dy
