...
class StationManagerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.station_manager.antenna = np.array(
            [
                AntennaBeamformingImt(par, 60, -10),
                AntennaBeamformingImt(par, 180, -10),
                AntennaBeamformingImt(par, 300, -10),
            ]
        )
    ...
    def test_tx_power(self):
        ...
        npt.assert_array_equal(
            self.station_manager.tx_power, dict({0: [33, 38], 1: [35], 2: [20, 25]})
        )
    ...
    def test_station(self):
        ...
        # test station type
        self.assertEqual(
            self.station_manager.get_station(0).station_type, StationType.IMT_BS
        )
    ...
    def test_distance_to(self):
        ...
        ref_distance = np.asarray(
            [[127.279, 302.200], [113.137, 288.140], [98.994, 274.089]]
        )
    ...
    def test_3d_distance_to(self):
        ...
        ref_distance = np.asarray(
            [[127.314, 302.226], [113.154, 288.156], [99, 274.096]]
        )
    ...
    def test_wrap_around(self):
        ...
        ref_d_2D = np.asarray([[18.03, 150.32, 85.39], [147.68, 181.12, 205.25]])
        npt.assert_allclose(d_2D, ref_d_2D, atol=1e-2)

        # 3D Distance
        ref_d_3D = np.asarray([[18.27, 150.33, 85.39], [147.73, 181.15, 205.26]])
        npt.assert_allclose(d_3D, ref_d_3D, atol=1e-2)

        # Point vec
        ref_phi = np.asarray([[56.31, -176.26, 103.55], [161.44, -56.49, 145.92]])
        npt.assert_allclose(phi, ref_phi, atol=1e-2)

        ref_theta = np.asarray([[99.45, 90.76, 90.67], [91.55, 90.95, 90.56]])
    ...
    def test_pointing_vector_to(self):
        ...
        npt.assert_allclose(
            phi, np.array([[45.00, 51.04], [45.00, 51.34], [45.00, 51.67]]), atol=eps
        )
        npt.assert_allclose(
            theta, np.array([[88.65, 89.24], [88.89, 89.40], [89.42, 89.58]]), atol=eps
        )
        ...
        npt.assert_allclose(
            phi,
            np.array([[-135.00, -135.00, -135.00], [-128.96, -128.66, -128.33]]),
            atol=eps,
        )
        npt.assert_allclose(
            theta, np.array([[91.35, 91.01, 90.58], [90.76, 90.60, 90.42]]), atol=eps
        )
