...
class AntennaBeamformingImtF1336Test(unittest.TestCase):
    ...
    def test_super_position_vector(self):
        ...
        npt.assert_allclose(np.real(v_vec), np.real(expected_v_vec), atol=1e-2)
        npt.assert_allclose(np.imag(v_vec), np.imag(expected_v_vec), atol=1e-2)
    ...
    def test_weight_vector(self):
        ...
        npt.assert_allclose(np.real(w_vec), np.real(expected_w_vec), atol=1e-2)
        npt.assert_allclose(np.imag(w_vec), np.imag(expected_w_vec), atol=1e-2)
    ...
    def test_calculate_gain(self):
        ...
        gains = self.antenna1.calculate_gain(phi_vec=phi_vec, theta_vec=theta_vec)
