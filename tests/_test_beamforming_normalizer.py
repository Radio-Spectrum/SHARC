...
from sharc.antenna.beamforming_normalization.beamforming_normalizer import (
    BeamformingNormalizer,
)
...
class BeamformingNormalizerTest(unittest.TestCase):
    ...
    def test_generate_correction_matrix(self):
        ...
        self.assertAlmostEqual(data["correction_factor_adj_channel"], 4.8, delta=1e-1)

        # Test 3.2: BS array
        npt.assert_allclose(
            data["correction_factor_co_channel"], np.array([[8.0], [8.0]]), atol=1e-1
        )
        data.close()
        os.remove(file_name)
