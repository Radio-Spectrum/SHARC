...
class PropagationHDFSSBuildingSide(Propagation):
    ...
    def __init__(
        ...
        self.propagation_p1411 = PropagationP1411(
            random_number_gen, above_clutter=False
        )
    ...
    def get_loss(self, *args, **kwargs) -> np.array:
        ...
        next_build = self.is_next_building(imt_x, imt_y, es_x, es_y)
        ...
        loss[:, same_build] += self.propagation_fspl.get_loss(
            distance_3D=d[:, same_build], frequency=f[:, same_build]
        )
    ...
    def is_same_building(self, imt_x, imt_y, es_x, es_y):
        ...
        building_y_range = (es_y - self.b_d / 2) + (1 + self.b_tol) * np.array(
    ...
    def is_next_building(self, imt_x, imt_y, es_x, es_y):
        same_building_x_range = es_x + (1 + self.b_tol) * np.array(
            [-self.b_w / 2, +self.b_w / 2]
        )
        same_building_y_range = (es_y - self.b_d / 2) + (1 + self.b_tol) * np.array(
            [-self.b_d / 2, +self.b_d / 2]
        )
