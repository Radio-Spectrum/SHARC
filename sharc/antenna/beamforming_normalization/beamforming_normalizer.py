...
class BeamformingNormalizer(object):
    ...
    def __init__(self, res_deg: float, tol: float):
        ...
        self.phi_vals_deg = np.arange(self.phi_min_deg, self.phi_max_deg, res_deg)
        self.theta_vals_deg = np.arange(self.theta_min_deg, self.theta_max_deg, res_deg)
        self.antenna = None

    def generate_correction_matrix(
        self, par: AntennaPar, file_name: str, testing=False
    ):
        """
        ...
        for phi_idx, phi in enumerate(self.phi_vals_deg):
            ...
            for theta_idx, theta in enumerate(self.theta_vals_deg):
                ...
                stdout.flush()
                (
                    correction_factor_co[phi_idx, theta_idx],
                    error_co[phi_idx, theta_idx],
    ...
    def calculate_correction_factor(
        self, phi_beam: float, theta_beam: float, c_chan: bool
    ):
        """
        ...
        if c_chan:
            ...
            int_f = lambda t, p: np.power(
                10, self.antenna._beam_gain(np.rad2deg(p), np.rad2deg(t), beam) / 10
            ) * np.sin(t)
        else:
            int_f = lambda t, p: np.power(
                10,
                self.antenna.element.element_pattern(np.rad2deg(p), np.rad2deg(t)) / 10,
            ) * np.sin(t)
...
if __name__ == "__main__":
    ...
    plt.plot(
        norm.theta_vals_deg,
        np.transpose(correction_factor),
        norm.theta_vals_deg,
        np.transpose(err_low),
        "r--",
        norm.theta_vals_deg,
        np.transpose(err_high),
        "r--",
    )
    plt.xlim(0, 180)
    plt.ylabel(r"Correction factor [dB]")
