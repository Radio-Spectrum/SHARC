...
class AntennaS1528(Antenna):
    ...
    def __init__(self, param: ParametersFssSs):
        ...
        # back-lobe level
        self.l_b = np.maximum(
            0, 15 + self.l_s + 0.25 * self.peak_gain + 5 * math.log10(self.z)
        )
    ...
    def calculate_gain(self, *args, **kwargs) -> np.array:
        ...
        idx_1 = np.where(
            (self.a * self.psi_b < psi) & (psi <= 0.5 * self.b * self.psi_b)
        )[0]
        gain[idx_1] = self.peak_gain + self.l_s + 20 * math.log10(self.z)

        idx_2 = np.where(
            (0.5 * self.b * self.psi_b < psi) & (psi <= self.b * self.psi_b)
        )[0]
...
if __name__ == "__main__":
    ...
    fig = plt.figure(
        figsize=(8, 7), facecolor="w", edgecolor="k"
