...
class SimulationIndoorTest(unittest.TestCase):
    ...
    def test_simulation_fss_es(self):
        ...
        npt.assert_allclose(self.simulation.bs_to_ue_phi, expected_azi, atol=1e-2)
        ...
        npt.assert_allclose(self.simulation.bs_to_ue_theta, expected_ele, atol=1e-2)
        ...
        for k in range(self.simulation.bs.num_stations):
            ...
            lo_angles = self.simulation.bs.antenna[k].to_local_coord(
                expected_azi[k, k], expected_ele[k, k]
            )
            npt.assert_array_almost_equal(lo_angles, expected_loc[k], decimal=2)
            npt.assert_array_almost_equal(
                self.simulation.bs.antenna[k].beams_list[0], expected_beam[k], decimal=2
            )
        ...
        for k in range(self.simulation.bs.num_stations):
            lo_angles = self.simulation.bs.antenna[k].to_local_coord(
                expected_phi_es[k], expected_theta_es[k]
            )
            npt.assert_array_almost_equal(lo_angles, expected_es_loc[k], decimal=2)

        # Test gain to ES
        calc_gain = self.simulation.calculate_gains(
            self.simulation.bs, self.simulation.system
        )
        for k in range(self.simulation.bs.num_stations):
            beam = 0
            exp_gain = self.simulation.bs.antenna[k]._beam_gain(
                expected_es_loc[k][0], expected_es_loc[k][1], beam
            )
            self.assertAlmostEqual(
                np.ndarray.item(calc_gain[k]), np.ndarray.item(exp_gain), places=1
            )
