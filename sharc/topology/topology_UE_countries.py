...
class TopologyUECountries(Topology):
    ...
    def __init__(self,
        ...
        super().__init__(
            getattr(bs_topology, "intersite_distance", 0.0),
            bs_topology.cell_radius,
        )
    ...
    def calculate_coordinates(self):
        """Place the UEs around every base station of the BS topology.

        TopologyCountries already resolves the base stations into the
        simulator's local ENU plane (East, North, height), so the UE offsets
        are simply added to the base station position on that same plane -- no
        ECEF round trip is needed, and none may be done, otherwise the UEs
        would not share the frame of the base stations serving them.
        """
        ...
        # Radius ~ uniform in area within the [R_guard, R] annulus
        u = self.rng.rand(nb, ue_per_bs)
        r = np.sqrt((R**2 - R_guard**2) * u + R_guard**2)

        # Angle around each BS azimuth, inside the sector
        bs_azimuth = np.asarray(self.bs_topology.azimuth, dtype=float)
        theta = np.deg2rad(
            bs_azimuth[:, np.newaxis]
            + self.rng.uniform(-half_bw, half_bw, size=(nb, ue_per_bs))
        )

        bs_x = np.asarray(self.bs_topology.x, dtype=float)[:, np.newaxis]
        bs_y = np.asarray(self.bs_topology.y, dtype=float)[:, np.newaxis]

        self.x = (bs_x + r * np.cos(theta)).ravel()
        self.y = (bs_y + r * np.sin(theta)).ravel()
        self.z = np.full(total_ue, h_mu, dtype=float)

        # Angle of the UE as seen from the BS, and the vertical angle between
        # the two, so that the UE may be pointed back at its serving station
        self.azimuth = np.degrees(theta).ravel()
        bs_height = np.broadcast_to(
            np.asarray(self.bs_topology.height, dtype=float).reshape(-1, 1),
            (nb, ue_per_bs),
        )
        self.ue_elevation_deg = np.degrees(
            np.arctan((bs_height - h_mu) / r)
        ).ravel()
        self.distance_to_bs = r.ravel()

        self.num_base_stations = total_ue
        self.indoor = np.zeros(total_ue, dtype=bool)
        self.elevation = np.zeros(total_ue, dtype=float)
