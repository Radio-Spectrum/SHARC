...
class SimulationUplinkTest(unittest.TestCase):
    def setUp(self):
        ...
        self.param.imt.line_of_sight_prob = (
            0.75  # probability of line-of-sight (not for FSPL)
        )
    ...
    def test_simulation_2bs_4ue_ss(self):
        ...
        self.simulation.propagation_imt = PropagationFactory.create_propagation(
            self.param.imt.channel_model, self.param, random_number_gen
        )
        self.simulation.propagation_system = PropagationFactory.create_propagation(
            self.param.fss_ss.channel_model, self.param, random_number_gen
        )

        # test coupling loss method
        self.simulation.coupling_loss_imt = self.simulation.calculate_coupling_loss(
            self.simulation.bs, self.simulation.ue, self.simulation.propagation_imt
        )
        coupling_loss_imt = np.array(
            [
                [88.47 - 1 - 10, 99.35 - 1 - 11, 103.27 - 1 - 22, 107.05 - 1 - 23],
                [107.55 - 2 - 10, 104.72 - 2 - 11, 101.53 - 2 - 22, 91.99 - 2 - 23],
            ]
        )
        npt.assert_allclose(
            self.simulation.coupling_loss_imt, coupling_loss_imt, atol=1e-2
        )
        ...
        npt.assert_allclose(
            self.simulation.ue.bandwidth, bandwidth_per_ue * np.ones(4), atol=1e-2
        )
        ...
        npt.assert_allclose(
            self.simulation.bs.rx_interference[0], rx_interference[0], atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.rx_interference[1], rx_interference[1], atol=1e-2
        )
        ...
        npt.assert_allclose(self.simulation.bs.thermal_noise, thermal_noise, atol=1e-2)

        # check BS thermal noise + interference
        total_interference = {
            0: 10
            * np.log10(
                np.power(10, 0.1 * rx_interference[0])
                + np.power(10, 0.1 * thermal_noise)
            ),
            1: 10
            * np.log10(
                np.power(10, 0.1 * rx_interference[1])
                + np.power(10, 0.1 * thermal_noise)
            ),
        }
        npt.assert_allclose(
            self.simulation.bs.total_interference[0], total_interference[0], atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.total_interference[1], total_interference[1], atol=1e-2
        )

        # check SNR
        npt.assert_allclose(
            self.simulation.bs.snr[0], rx_power[0] - thermal_noise, atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.snr[1], rx_power[1] - thermal_noise, atol=1e-2
        )

        # check SINR
        npt.assert_allclose(
            self.simulation.bs.sinr[0], rx_power[0] - total_interference[0], atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.sinr[1], rx_power[1] - total_interference[1], atol=1e-2
        )
        ...
        self.assertAlmostEqual(
            self.simulation.system.rx_interference, rx_interference, delta=0.01
        )

        # check FSS space station thermal noise
        thermal_noise = 10 * np.log10(1.38064852e-23 * 950 * 100 * 1e3 * 1e6)
        self.assertAlmostEqual(
            self.simulation.system.thermal_noise, thermal_noise, delta=0.01
        )

        # check INR at FSS space station
        self.assertAlmostEqual(
            self.simulation.system.inr, rx_interference - thermal_noise, delta=0.01
        )
    ...
    def test_simulation_2bs_4ue_es(self):
        ...
        ext_interference = {
            0: system_tx_power - coupling_loss_imt_system[0:2],
            1: system_tx_power - coupling_loss_imt_system[2:4],
        }
        npt.assert_allclose(
            self.simulation.bs.ext_interference[0], ext_interference[0], atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.ext_interference[1], ext_interference[1], atol=1e-2
        )
        ...
        npt.assert_allclose(
            self.simulation.bs.sinr_ext[0], rx_power[0] - interference[0], atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.sinr_ext[1], rx_power[1] - interference[1], atol=1e-2
        )

        # INR
        npt.assert_allclose(
            self.simulation.bs.inr[0], interference[0] - thermal_noise, atol=1e-2
        )
        npt.assert_allclose(
            self.simulation.bs.inr[1], interference[1] - thermal_noise, atol=1e-2
        )
    ...
    def test_simulation_2bs_4ue_ras(self):
        ...
        npt.assert_allclose(
            self.simulation.coupling_loss_imt_system,
            np.array(
                [125.55 - 50 - 10, 125.76 - 50 - 11, 125.93 - 50 - 22, 126.17 - 50 - 23]
            ),
            atol=1e-2,
        )

        # Test RAS PFD
        interference = 20 - np.array(
            [125.55 - 50 - 10, 125.76 - 50 - 11, 125.93 - 50 - 22, 126.17 - 50 - 23]
        )
        rx_interference = 10 * math.log10(np.sum(np.power(10, 0.1 * interference)))
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(self.simulation.system.pfd, pfd, delta=0.01)
    ...
    def test_beamforming_gains(self):
        ...
        for i in range(self.simulation.ue.num_stations):
            self.simulation.ue.antenna[i] = AntennaBeamformingImt(
                par, self.simulation.ue.azimuth[i], self.simulation.ue.elevation[i]
            )
        ...
        npt.assert_allclose(
            phi,
            np.array([[0.0, 30.0, 0.0, 13.898], [180.0, 170.935, 180.0, 120.0]]),
            atol=eps,
        )
        npt.assert_allclose(
            theta,
            np.array(
                [[95.143, 95.143, 91.718, 91.430], [91.718, 91.624, 95.143, 95.143]]
            ),
            atol=eps,
        )
        ...
        # Test beams pointing
        npt.assert_allclose(
            self.simulation.bs.antenna[0].beams_list[0], (0.0, -4.857), atol=eps
        )
        npt.assert_allclose(
            self.simulation.bs.antenna[0].beams_list[1], (29.92, -3.53), atol=eps
        )
        npt.assert_allclose(
            self.simulation.bs.antenna[1].beams_list[0], (0.0, -4.857), atol=eps
        )
        npt.assert_allclose(
            self.simulation.bs.antenna[1].beams_list[1], (-59.60, 0.10), atol=eps
        )
        npt.assert_allclose(
            self.simulation.ue.antenna[0].beams_list[0], (0.0, -35.143), atol=eps
        )
        npt.assert_allclose(
            self.simulation.ue.antenna[1].beams_list[0], (-62.04, -12.44), atol=eps
        )
        npt.assert_allclose(
            self.simulation.ue.antenna[2].beams_list[0], (-88.66, -4.96), atol=eps
        )
        npt.assert_allclose(
            self.simulation.ue.antenna[3].beams_list[0], (32.16, 20.71), atol=eps
        )

        # BS Gain matrix
        ref_gain = np.array([[34.03, 32.37, 8.41, -9.71], [8.41, -8.94, 34.03, 27.42]])
        gain = self.simulation.calculate_gains(self.simulation.bs, self.simulation.ue)
        npt.assert_allclose(gain, ref_gain, atol=eps)

        # UE Gain matrix
        ref_gain = np.array(
            [[4.503, -44.198], [-3.362, -11.206], [-14.812, -14.389], [-9.726, 3.853]]
        )
