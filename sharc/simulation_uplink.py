...
class SimulationUplink(Simulation):
    ...
    def calculate_sinr(self):
        ...
        if self.parameters.imt.central_bs_interference_only and self.parameters.imt.topology.type == "MACROCELL":
            # we'll calculate interference only into the central BS.
            bs_active = self.topology.central_cell_idx
        else:
            bs_active = np.where(self.bs.active)[0]
    ...
    def calculate_sinr_ext(self):
        ...
        if self.parameters.imt.central_bs_interference_only and self.parameters.imt.topology.type == "MACROCELL":
            # we'll calculate interference only into the central BS.
            self.bs.active[:] = False
            self.bs.active[self.topology.central_cell_idx] = True

        bs_active = np.where(self.bs.active)[0]

        if self.co_channel:
            # coupling-loss between system's in-band transmission into IMT in-band reception.
        ...
        if self.adjacent_channel:
            # coupling-loss from system's out-of-band emissions into IMT in-band reception
            if self.param_system.adjacent_ch_emissions != "OFF":
                # TODO: Store this for results later
                self.coupling_loss_oob_tx_inband_rx = self.calculate_coupling_loss_system_imt(
                    self.system,
                    self.bs,
                    system_inband=False,
                    imt_inband=True,
                )
            # coupling-loss from system's in-band transmission into IMT out-of-band reception
            # (e.g. due to reception filter imperfections).
        ...
        for bs in bs_active:
            ...
            if self.adjacent_channel:
                ...
                if self.param_system.adjacent_ch_emissions != "OFF":
                    ...
                    tx_oob = tx_oob - self.coupling_loss_oob_tx_inband_rx[active_beams, :][:, system_interfering]
            ...
            self.bs.total_rx_interference[bs] = 10 * np.log10(
                np.power(10, 0.1 * self.bs.rx_interference[bs]) + np.power(10, 0.1 * self.bs.ext_interference[bs]))
    ...
    def calculate_external_interference(self):
        ...
        if self.co_channel:
            # coupling-loss between IMT in-band transmission into system in-band reception.
            self.coupling_loss_imt_system = self.calculate_coupling_loss_system_imt(
                self.system,
                self.ue,
                system_inband=True,
        ...
        if self.adjacent_channel:
            # coupling-loss from IMT's out-of-band emissions into System in-band reception
            ...
            # coupling-loss from IMT's in-band transmission into Systems out-of-band reception
            # (e.g. due to reception filter imperfections).
            self.coupling_loss_imt_system_adjacent = self.calculate_coupling_loss_system_imt(
                self.system,
                self.ue,
        ...
        for bs in bs_active:
            ues = self.link[bs]
            for ue in ues:
                sys_victim = np.where(ue_interf_from_sys_paths_mask[ue])[0]
                if len(sys_victim) == 0:
                    continue
                ...
                if self.co_channel:
                    ...
                    interference_ue = self.ue.tx_power[ue] - \
                        self.coupling_loss_imt_system[ue, sys_victim]
                ...
                if self.adjacent_channel:
                    ...
                    # Out of band power
                    if self.parameters.imt.adjacent_ch_emissions != "OFF":
                        tx_oob -= self.coupling_loss_oob_tx_inband_rx[ue, sys_victim]

                    if self.param_system.adjacent_ch_reception != "OFF":
                        rx_oob -= self.coupling_loss_imt_system_adjacent[ue, sys_victim]
                    # Out of band power
    ...
    def collect_results(self, write_to_file: bool, snapshot_number: int):
        ...
        if not self.parameters.imt.interfered_with and np.any(self.bs.active):
            self.results.system_inr.extend(self.system.inr[sys_active].tolist())
            # FIXME: self.system.rx_interference should not be a scalar
            self.results.system_ul_interf_power.extend(
                [self.system.rx_interference],
            )
            self.results.system_ul_interf_power_per_mhz.extend(
                [self.system.rx_interference
                    - 10 * math.log10(self.system.bandwidth[sys_active])],
            )
            # TODO: generalize this a bit more if needed
            if hasattr(
                    self.system.antenna[0],
                    "effective_area") and self.system.num_stations == 1:
                self.results.system_pfd.extend([self.system.pfd[sys_active]])

        self.add_system_imt_interaction_attr_to_results("UL", "system_imt_antenna_gain")
        if len(self.imt_system_antenna_gain):
            self.add_system_imt_interaction_attr_to_results("UL", "imt_system_antenna_gain")

        self.add_system_imt_interaction_attr_to_results("UL", "imt_system_path_loss")

        if len(self.imt_system_antenna_gain_adjacent):
            self.add_system_imt_interaction_attr_to_results("UL", "imt_system_antenna_gain_adjacent")
        if len(self.system_imt_antenna_gain_adjacent):
            self.add_system_imt_interaction_attr_to_results("UL", "system_imt_antenna_gain_adjacent")
        if self.param_system.channel_model == "HDFSS":
            self.add_system_imt_interaction_attr_to_results("UL", "imt_system_path_loss")
            self.add_system_imt_interaction_attr_to_results("UL", "imt_system_build_entry_loss")
            self.add_system_imt_interaction_attr_to_results("UL", "imt_system_diffraction_loss")
        ...
        for bs in bs_active:
            ...
            # if we're only interested in interference into the central BS, we can skip calculating tput for the other BSs
            if self.parameters.imt.central_bs_interference_only and self.parameters.imt.topology.type == "MACROCELL":
                central_bs_idx = self.topology.central_cell_idx
                if bs not in central_bs_idx:
                    continue
            ...
            if self.parameters.imt.interfered_with:
                ...
                self.results.imt_ul_intra_interf_power.extend(
                    (self.bs.rx_interference[bs] - 30).tolist(),
                )

                self.results.imt_ul_total_interf_power.extend(
                    (self.bs.total_rx_interference[bs] - 30).tolist(),
                )
